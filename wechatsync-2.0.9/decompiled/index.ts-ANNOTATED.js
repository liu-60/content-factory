// ============================================================================
// WechatSync v2.0.9 - Chrome 扩展后台 Service Worker（反编译美化版）
// ============================================================================
// 
// 【整体架构概览】
// 本文件是 WechatSync 浏览器扩展的主后台 Service Worker，负责：
//   1. 管理 20+ 个内容平台的适配器（知乎、掘金、微信公众号、微博、头条、B站等）
//   2. 文章内容的提取 → 预处理 → 图片上传 → 平台发布的完整数据流
//   3. Chrome 扩展消息系统的统一处理（popup/content-script/MCP 客户端通信）
//   4. MCP (Model Context Protocol) WebSocket 客户端，支持外部 AI 工具调用
//   5. WordPress/Typecho/MetaWeblog 等 CMS 系统的 XML-RPC 发布
//   6. 同步历史记录管理、认证状态缓存、远程配置拉取
//
// 【数据流】
//   文章提取(content-script) → HTML/Markdown 预处理 → 图片批量上传到目标平台
//   → 内容格式转换(Draft.js/ProseMirror/HTML) → 调用平台 API 保存草稿
//
// 【文件结构（按行号范围）】
//   L1-17:      外部模块导入（JSZip, logger, version-check, remote-config）
//   L49-61:     默认预处理器配置
//   L63-163:    Markdown 图片解析工具函数
//   L164-328:   平台适配器基类（BaseAdapter）
//   L329-377:   适配器注册表（AdapterRegistry）
//   L378-702:   内容格式转换（Markdown→Draft.js / Markdown→富文本）
//   L703-771:   AWS Signature V4 签名（字节跳动 ImageX 上传用）
//   L792-7300:  各平台适配器实现（20+个平台）
//   L7489-7674: Chrome 扩展运行时封装（ExtensionRuntime）
//   L7692-7735: 适配器注册初始化
//   L7736-7875: 认证检查与缓存管理
//   L7876-7976: 同步编排器（多平台并行发布）
//   L7977-8690: WordPress / MetaWeblog XML-RPC 实现
//   L8691-9031: 同步服务（历史记录、状态管理、内容预处理管线）
//   L9032-9316: MCP WebSocket 客户端（外部 AI 工具集成）
//   L9332-10155: Chrome 消息处理器（所有消息类型的统一分发）
//   L10157-10316: 生命周期管理（安装/更新/启动/定时任务）
// ============================================================================

// ============================================================================
// 【模块导入】
// N/Mt/tr/rr/sr/ar/nr = JSZip 库的各种导出（用于 ZIP 打包下载功能）
// Te = 日志记录器（logger），提供 debug/info/warn/error 级别日志
// or/De/ir/cr/ur/lr/dr/hr/jt/mt/et/pr = 版本检查和远程配置模块
//   - De: 当前扩展版本号
//   - mt: 获取远程版本信息
//   - et: 检查更新
//   - jt/mt: 版本比较和安装引导
...
import {
    f as Ft,
    a as _r
} from "./remote-config-BEX-YVxx.js";
import "./_commonjsHelpers-BosuxZz1.js";
// ============================================================================
// 【默认预处理器配置】
// 控制内容从源平台提取后的清洗和转换行为：
//   - outputFormat: 输出格式，默认 'markdown'（部分适配器会覆盖为 'html'）
//   - removeIframes: 移除嵌入的 iframe（如视频播放器）
//   - removeComments: 移除 HTML 注释
//   - removeScripts: 移除 <script> 标签
//   - removeStyles: 移除 <style> 标签
//   - processCodeBlocks: 处理代码块（转为 Markdown 围栏格式）
//   - processLazyImages: 处理懒加载图片（将 data-src 替换为 src）
//   - processTables: 处理表格（转为 Markdown 表格语法）
//   - processLinks: 处理链接（清理追踪参数等）
// ============================================================================
const Ir = {
    outputFormat: "html",
    removeIframes: !0,
...
// ============================================================================
// 【Markdown 图片解析器 - xe()】
// 解析 Markdown 文本中的图片语法 ![alt](src)，返回图片对象数组。
// 支持嵌套括号的正确匹配（如 URL 中包含括号的情况）。
// 输入: Markdown 文本字符串
// 输出: [{ full: '![alt](src)', alt: '替代文本', src: '图片URL' }, ...]
// 用途: 在基类的 processImages() 中被调用，提取所有需要上传的图片
// ============================================================================
function xe(c) {
    const o = [],
        e = c.length;
...
// ============================================================================
// 【平台适配器基类 - D (BaseAdapter)】
// 所有平台适配器的父类，提供通用功能：
//   - init(runtime): 注入 Chrome 运行时封装实例
//   - addHeaderRule(): 添加 declarativeNetRequest 请求头规则（伪造 Origin/Referer）
//   - withHeaderRules(): 包装 API 调用，自动添加/清理请求头规则
//   - get()/postJson()/postForm()/postMultipart(): HTTP 请求方法封装
//   - processImages(html, markdown, uploadFn): 核心图片处理管线
//     遍历 HTML <img> 和 Markdown ![]() 中的图片，调用平台上传函数，替换 URL
//   - uploadImageByUrl(url): 子类必须实现，将 data URI 图片上传到平台 CDN
//   - createResult(success, extra): 生成统一的同步结果对象
// 子类需要定义:
//   - static meta = { id, name, icon }  平台元信息
//   - checkAuth(): 检查登录状态
//   - publish(article): 发布文章到平台
//   - uploadImageByUrl(): 图片上传实现
// ============================================================================
...
class D {
    ...
    async get(o, e) {
        const t = await this.runtime.fetch(o, {
            method: "GET",
            credentials: "include",
            headers: e
        });
    ...
    async processImages(o, e, t) {
        ...
        let a;
        for (;
            (a = i.exec(o)) !== null;) n.push({
            full: a[0],
            src: a[1],
            type: "html"
        });
        for (const d of xe(o)) n.push({
            full: d.full,
            src: d.src,
            alt: d.alt,
            type: "markdown"
        ...
        for (const {
            ...
            if (m) {
                ...
                try {
                    ...
                    let _;
                    if (p === "html") {
                        if (_ = `<img src="${w.url}"`, w.attrs)
                            for (const [y, T] of Object.entries(w.attrs)) _ += ` ${y}="${T}"`;
                        _ += " />"
                    } else _ = `![${f||""}](${w.url})`;
...
// ============================================================================
// 【适配器注册表 - Sr (AdapterRegistry)】
// 单例模式，管理所有平台适配器的注册和获取：
//   - register(adapterClass): 注册适配器类（传入类本身，非实例）
//   - get(id): 懒实例化——首次获取时 new 适配器并注入 runtime，后续直接返回缓存实例
//   - getAllMeta(): 返回所有已注册适配器的元信息 [{ id, name, icon }]
//   - getPreprocessConfig(id): 获取特定适配器的预处理器配置
//   - setRuntime(runtime): 设置运行时实例（会清空已缓存的适配器实例）
// 全局单例: fe = new Sr()
// ============================================================================
class Sr {
    constructor() {
    ...
    getPreprocessConfig(o) {
        ...
        return {
            ...Ir,
            ...(e == null ? void 0 : e.preprocessConfig) || {}
        }
    }
...
function vr(c, o, e) {
    ...
    return c.children.forEach(function(i) {
        if (i.type === "text") t += i.content;
        else if (i.type === "softbreak") t += `
`;
...
`;
            ...
            var a = bt();
            r[a] = o[i.type](i), s.push({
                offset: ke(t) || 0,
                length: 0,
                key: a
            })
        } else if (i.type.indexOf("_close") !== -1 && o[i.type.replace("_close", "_open")]) s[s.length - 1].length = ke(t) - s[s.length - 1].offset;
        else if (i.type.indexOf("_close") !== -1 && e[i.type.replace("_close", "_open")]) {
        ...
        blockEntities: r,
        blockEntityRanges: s,
        blockInlineStyleRanges: n
    }
}

// ============================================================================
// 【Markdown → Draft.js 转换器 - Lt()】
// 将 Markdown 文本转换为 Facebook Draft.js 编辑器所需的 JSON 格式。
// 使用 Remarkable 库解析 Markdown AST，然后映射为 Draft.js 的 Block 结构：
//   - paragraph_open → unstyled 块
//   - heading_open → header-one/two/three 块
//   - fence → code-block 块（保留语言标记）
//   - bullet_list/ordered_list → unordered/ordered-list-item 块
//   - blockquote → blockquote 块
// 内联样式处理（vr() 辅助函数）：
//   - strong → BOLD 样式范围
//   - em → ITALIC 样式范围
//   - code → CODE 样式范围
//   - del → STRIKETHROUGH 样式范围
// 实体处理：
//   - link → LINK 实体（MUTABLE）
//   - image → IMAGE 实体（IMMUTABLE，包含上传后的 URL 映射）
// 使用者: 豆瓣适配器、X/Twitter 适配器（需要 Draft.js 格式内容的平台）
// ============================================================================
function Lt(c) {
    var o = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {},
        e = o.remarkablePreset || o.remarkableOptions,
        t = Xe(o.remarkableOptions) === "object" ? o.remarkableOptions : null,
        r = new Mt(e, t);
    if ((!t || !t.enable || !t.enable.block || t.enable.block !== "table" || t.enable.block.includes("table") === !1) && r.block.ruler.disable("table"), t && t.disable)
        for (var s = 0, n = Object.entries(t.disable); s < n.length; s++) {
            var i = wt(n[s], 2),
                a = i[0],
                u = i[1];
            r[a].ruler.disable(u)
    ...
    if (t && t.enable)
        for (var l = 0, h = Object.entries(t.enable); l < h.length; l++) {
            var d = wt(h[l], 2),
                m = d[0],
                f = d[1];
            r[m].ruler.enable(f)
    ...
    var p = [],
        ...
        y = null,
        T = 0,
        P = Object.assign({}, tt, o.blockTypes || {}),
        A = Object.assign({}, Ar, o.blockEntities || {}),
        S = Object.assign({}, Er, o.blockStyles || {});
    return _.forEach(function(v) {
        ...
        var U = v.type;
        if (U === "list_item_open" && (U = y), U === "inline") {
            var k = vr(v, A, S),
                O = k.content,
                b = k.blockEntities,
                I = k.blockEntityRanges,
                C = k.blockInlineStyleRanges,
                F = p[p.length - 1];
            F.text = O, F.inlineStyleRanges = C, F.entityRanges = I, Object.assign(w, b)
        } else if ((U.indexOf("_open") !== -1 || U === "fence" || U === "hr" || U === "htmlblock") && P[U]) {
            var J = 0,
                $;
            if (v.level > 0 && (J = Math.floor(v.level / 2)), v.level === 0 || v.type === "list_item_open" ? $ = Object.assign({
                    depth: J
                }, P[U](v)) : v.level > 0 && p[p.length - 1].text && ($ = Object.assign({}, p[p.length - 1])), $ && o.preserveNewlines)
                for (var x = v.lines[0] - T, E = 0; E < x; E++) p.push(tt.paragraph_open());
            $ && (T = v.lines[1], ($.type === "unordered-list-item" || $.type === "ordered-list-item") && (T += 1), p.push($))
        }
    }), p.length || p.push(tt.paragraph_open()), {
        entityMap: w,
        blocks: p
    }
}
...
// ============================================================================
// 【Markdown → 富文本 JSON 转换器 - $r()】
// 将 Markdown 转换为豆瓣/微信公众号使用的富文本 JSON 格式。
// 输入: Markdown 文本 + 图片映射表 Map<src, uploadedInfo>
// 输出: JSON 字符串，包含 type/text/attrs 的嵌套结构
// 支持的元素:
//   - heading (#~######) → heading 节点
//   - paragraph → text 节点
//   - blockquote → blockquote 节点
//   - code block (```) → pre/code 节点
//   - unordered/ordered list → list 节点
//   - image ![alt](src) → image 节点（使用上传后的 URL）
//   - bold/italic/code/strikethrough → 内联样式标签
//   - link [text](url) → a 节点
// ============================================================================
...
`);
    ...
    const r = () => t++,
        s = Lt(e, {
            ...
            blockEntities: {
                image: function(n) {
                    const i = n.src ? n.src.split("?#") : ["", ""],
                        a = i[0],
                        u = i[1] || "",
                        l = o.get(n.src) || o.get(a);
                    return l ? {
                        type: "IMAGE",
                        mutability: "IMMUTABLE",
                        data: {
    ...
    if (s.blocks)
        for (const n of s.blocks) n.blockEntities && (Object.assign(s.entityMap, n.blockEntities), delete n.blockEntities);
    return JSON.stringify(s)
}
// ============================================================================
// 【AWS Signature V4 签名工具】
// 用于字节跳动 ImageX / TOS 上传 API 的请求签名。
// Ae(data, key): 使用 Web Crypto API 进行 HMAC-SHA256 签名
// Pr(date): 将 Date 格式化为 AWS 日期字符串 YYYYMMDD
// Ve(config): 完整的 AWS SigV4 签名流程：
//   1. 构造规范请求（Canonical Request）
//   2. 构造待签名字符串（String to Sign）
//   3. 派生签名密钥（HMAC 链：Date → Region → Service → aws4_request）
//   4. 计算签名并生成 Authorization 头
// 使用者: 掘金适配器（Juejin）、抖音适配器（Douyin）
// ============================================================================
async function Ae(c, o) {
    const e = c instanceof Uint8Array ? c : new Uint8Array(c),
...
async function _t(c) {
    const o = new TextEncoder().encode(c),
        e = await crypto.subtle.digest("SHA-256", o);
    return Ht(e)
}
...
async function Ve(c) {
    const {
        ...
        headers: a = {},
        body: u = ""
    } = c, l = new URL(e), h = l.pathname, d = l.search.slice(1), m = new Date, f = Or(m), p = Pr(m), w = new URLSearchParams(d), y = Array.from(w.entries()).sort((M, re) => M[0].localeCompare(re[0])).map(([M, re]) => `${encodeURIComponent(M)}=${encodeURIComponent(re)}`).join("&"), T = {
        "x-amz-date": f
    };
    s && (T["x-amz-security-token"] = s), Object.assign(T, a);
    const P = Object.keys(T).map(M => M.toLowerCase()).sort().join(";"),
        A = Object.entries(T).map(([M, re]) => `${M.toLowerCase()}:${re.trim()}`).sort().join(`
`) + `
`,
        S = await _t(u),
        v = [o.toUpperCase(), h || "/", y, A, P, S].join(`
`),
        U = "AWS4-HMAC-SHA256",
        k = `${p}/${n}/${i}/aws4_request`,
        O = await _t(v),
        b = [U, f, k, O].join(`
`),
        I = await Ae(new TextEncoder().encode("AWS4" + r), p),
        C = await Ae(I, n),
        F = await Ae(C, i),
        J = await Ae(F, "aws4_request"),
        $ = await Ae(J, b),
        x = Ht($),
        E = `${U} Credential=${t}/${k}, SignedHeaders=${P}, Signature=${x}`,
        q = {
            authorization: E,
            "x-amz-date": f
        };
    return s && (q["x-amz-security-token"] = s), {
        authorization: E,
        amzDate: f,
        headers: q
    }
...
// ============================================================================
// 【CRC32 校验工具】
// qt(data): 计算数据的 CRC32 校验值，返回十六进制字符串
// Dr(): 懒初始化 CRC32 查找表（256 项 Uint32Array），缓存到全局变量 Ee
// 用途: TOS 上传时验证文件完整性（掘金适配器的图片上传流程）
// ============================================================================
function qt(c) {
    let o = 4294967295;
    const e = Dr();
...
// ============================================================================
// 【豆瓣适配器 - Mr (DoubanAdapter)】
// 平台: 豆瓣日记 (douban.com/note/create)
// 能力: article, draft, image_upload
// 输出格式: Markdown（预处理后转为 Draft.js JSON）
// 认证检查: 访问 douban.com 首页 HTML，提取 ck（cookie key）和用户信息
// 图片上传: multipart/form-data 上传到 douban.com/j/article/upload_photo
// 发布流程:
//   1. processImages() 上传所有图片到豆瓣 CDN
//   2. 调用 Lt() 将 Markdown 转为 Draft.js 格式
//   3. POST 到 /j/note/save 接口保存草稿
// ============================================================================
const we = N("Douban");
class Mr extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            const t = await (await this.runtime.fetch("https://www.douban.com/note/create", {
                ...
                r = t.match(/_USER_NAME\s*=\s*['"]([^'"]+)['"]/),
                s = t.match(/_USER_AVATAR\s*=\s*['"]([^'"]+)['"]/),
                n = t.match(/name="note_id"\s+value="(\d+)"/),
                i = t.match(/name="ck"\s+value="([^"]+)"/),
                a = t.match(/_POST_PARAMS\s*=\s*(\{[\s\S]*?\});/);
            if (!r || !n || !i) return {
                isAuthenticated: !1
            };
            if (this.username = r[1], this.avatar = s ? s[1] : "", this.formData = {
                    note_id: n[1],
                    ck: i[1]
                }, a) try {
            ...
            return we.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const n = $r(r, s),
                a = await (await this.runtime.fetch("https://www.douban.com/j/note/autosave", {
                    method: "POST",
                    credentials: "include",
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
...
// ============================================================================
// 【雪球适配器 - jr (XueqiuAdapter)】
// 平台: 雪球专栏 (xueqiu.com)
// 能力: article, draft, image_upload
// 输出格式: Markdown
// 认证检查: GET xueqiu.com/statuses/original/show.json，根据响应判断登录态
// 图片上传: multipart 上传到 xueqiu.com/statuses/upload_photo.json
// 发布流程: 自定义 Remarkable 渲染器将 Markdown 转为雪球 HTML 格式，POST 保存草稿
// ============================================================================
const ve = N("Xueqiu");
class jr extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            ...
            try {
                ...
                return ve.error(" Failed to parse user data:", s), {
                    isAuthenticated: !1
                }
            }
        } catch (e) {
            return ve.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
...
`).trim();
            ...
            const h = l.id,
                d = `https://mp.xueqiu.com/write/draft/${h}`;
    ...
    async uploadImageByUrl(e) {
        ...
        return {
            url: i.url.startsWith("//") ? `https:${i.url}/${i.filename}` : `${i.url}/${i.filename}`
        }
    }
}
// ============================================================================
// 【搜狐号适配器 - Nr (SohuAdapter)】
// 平台: 搜狐号 (mp.sohu.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: GET mp.sohu.com/api/user/info，解析 JSON 响应
// 设备ID: 生成 32 位随机十六进制字符串作为设备标识
// 子账号: 支持通过 subAccountId 发布到不同子账号
// 图片上传: multipart 上传到搜狐图片服务器
// 发布流程: HTML 内容 + 图片 URL 替换 → POST 保存草稿
// ============================================================================
const ae = N("Sohu");
...
class Nr extends D {
    ...
    async checkAuth() {
        ...
        try {
            ...
            if (ae.debug("checkAuth response:", i), i.code !== 2e6 || !((s = (r = (t = (e = i.data) == null ? void 0 : e.data) == null ? void 0 : t[0]) == null ? void 0 : r.accounts) != null && s.length)) return {
                isAuthenticated: !1
            };
            const a = [];
            ...
            return ae.debug("checkAuth: not logged in -", n), {
                isAuthenticated: !1,
                error: n.message
            }
        }
...
// ============================================================================
// 【人人都是产品经理适配器 - Fr (WoshipmAdapter)】
// 平台: 人人都是产品经理 (woshipm.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: 访问 woshipm.com 提取 jltoken（JWT-like token）
// 图片上传: 又拍云(upyun) CDN 上传，使用平台提供的签名
// 发布流程: HTML 内容 + 图片 URL 替换 → POST 保存草稿
// ============================================================================
const ue = N("Woshipm");
class Fr extends D {
    ...
    async checkAuth() {
        ...
        try {
            ...
            const a = i[1],
                l = await (await this.runtime.fetch(`https://www.woshipm.com/api2/user/profile?uid=${a}`, {
                    method: "GET",
                    credentials: "include",
                    headers: {
                        "X-Requested-With": "XMLHttpRequest"
            ...
            return ue.debug("checkAuth: not logged in -", r), {
                isAuthenticated: !1,
                error: r.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const a = String(i.post_id),
                u = i.url || `https://www.woshipm.com/writing?pid=${a}`;
    ...
    async uploadImageByUrl(e) {
        try {
            const t = await this.runtime.fetch(e, {
                credentials: "omit"
            });
            if (!t.ok) throw new Error(`Failed to fetch image: ${t.status}`);
            const r = await t.blob();
            return {
                url: await this.uploadImageBinaryInternal(r, this.getFilenameFromUrl(e))
            }
        } catch (t) {
            return ue.warn("Failed to upload image by URL:", e, t), {
                url: e
            }
        }
    }
...
// ============================================================================
// 【知乎适配器 - Hr (ZhihuAdapter)】
// 平台: 知乎专栏 (zhuanlan.zhihu.com)
// 能力: article, draft, image_upload
// 输出格式: HTML（ extensive 预处理）
// 预处理特殊配置:
//   - 表格转换为知乎兼容格式
//   - 图片包裹为 <figure> 标签（知乎编辑器要求）
//   - 代码块特殊处理（保留语言标记）
//   - 移除特定宽高的 img 属性
// 认证检查: GET zhuanlan.zhihu.com/api/articles/drafts 判断登录态
// 图片上传: 阿里云 OSS 直传
//   - HMAC-SHA1 签名（使用 accessKeyId/accessKeySecret）
//   - 上传后轮询图片就绪状态（知乎 CDN 有处理延迟）
// 发布流程: 内容转换 + 图片上传 → 创建/更新知乎草稿
// ============================================================================
const Q = N("Zhihu"),
    Lr = tr;
class Hr extends D {
    ...
    async checkAuth() {
        try {
            ...
            return Q.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    transformTables(e) {
        ...
        return t = t.replace(/<table[^>]*>([\s\S]*?)<\/table>/gi, (r, s) => {
            const n = s.match(/<thead[^>]*>([\s\S]*?)<\/thead>/i),
                i = s.match(/<tbody[^>]*>([\s\S]*?)<\/tbody>/i);
            let a = "",
                u = "";
    ...
    async uploadImageByUrl(e) {
        if (e.startsWith("data:")) {
            ...
            return {
                url: await this.uploadImageBinaryInternal(s)
            }
        }
    ...
    async waitForImageReady(e) {
        for (let r = 0; r < 10; r++) {
            const n = await (await this.runtime.fetch(`https://api.zhihu.com/images/${e}`, {
                credentials: "include"
            })).json();
    ...
    async ossUpload(e, t, r, s) {
        const n = r.type || "application/octet-stream",
            i = `${e}/${t}`,
...
` + h + `
` + m,
            p = await this.hmacSha1Base64(s.access_key, f),
            w = `OSS ${s.access_id}:${p}`;
    ...
    async hmacSha1Base64(e, t) {
        const r = new TextEncoder,
            s = r.encode(e),
            n = r.encode(t),
            i = await crypto.subtle.importKey("raw", s, {
                name: "HMAC",
                hash: "SHA-1"
            }, !1, ["sign"]),
...
// ============================================================================
// 【掘金适配器 - Br (JuejinAdapter)】
// 平台: 稀土掘金 (juejin.cn)
// 能力: article, draft, image_upload, category
// 输出格式: Markdown
// 认证检查: GET juejin.cn/api 验证 Cookie 中的 sessionid
// CSRF 管理: 从 Cookie 提取 __tea_cookie_tokens_2608 中的 csrf_token
// 图片上传（复杂流程）:
//   1. 获取 ImageX 上传 token（字节跳动图片服务）
//   2. 通过 TOS (Tinder Object Storage) 分片上传
//   3. CRC32 校验文件完整性
//   4. 调用 commit 接口获取最终 URL
// 分类获取: GET /api/article_category/by_type 获取文章分类列表
// 发布流程: Markdown + 图片上传 → 保存草稿（支持分类选择）
// ============================================================================
const B = N("Juejin"),
    It = "2608",
    St = "73owjymdk6";
...
class Br extends D {
    ...
    async checkAuth() {
        ...
        try {
            ...
            return B.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async uploadImageByUrl(e) {
        try {
            let t;
            if (e.startsWith("data:")) B.debug("Detected data URI, converting to blob"), t = await fetch(e).then(s => s.blob());
            else {
                B.debug("Downloading remote image:", e.substring(0, 80));
                const s = await this.runtime.fetch(e, {
                    method: "GET"
                });
                if (!s.ok) return B.warn("Failed to download image:", s.status), {
                    url: e
                };
                t = await s.blob()
            }
            const r = await this.uploadImageBinaryInternal(t);
            return B.debug("Uploaded image:", e.substring(0, 50), "->", r), {
                url: r
            }
        } catch (t) {
            return B.warn("Failed to upload image by URL:", e, t), {
                url: e
            }
        }
    }
    ...
    async applyImageUpload(e) {
        ...
        const t = `https://imagex.bytedanceapi.com/?Action=ApplyImageUpload&Version=2018-08-01&ServiceId=${St}`,
            ...
            n = await (await this.runtime.fetch(t, {
                method: "GET",
                headers: {
                    ...r.headers
                }
    ...
    async uploadToTOS(e, t) {
        const r = e.StoreInfos[0],
            s = e.UploadHosts[0];
        if (!r || !s) throw new Error("Invalid upload address");
        const n = `https://${s}/${r.StoreUri}`,
    ...
    async uploadImageBinaryInternal(e) {
        var a, u, l;
        const t = await this.getImageXToken(),
            r = await this.applyImageUpload(t);
...
// ============================================================================
// 【CSDN 适配器 - zr (CSDNAdapter)】
// 平台: CSDN 博客 (blog.csdn.net)
// 能力: article, draft, image_upload
// 输出格式: Markdown
// API 签名: 请求需要 HMAC-SHA256 签名（使用 API_KEY + API_SECRET）
//   - 签名头: x-ca-key, x-ca-nonce, x-ca-signature, x-ca-signature-headers
// UUID: 每个请求生成唯一 UUID 用于追踪
// 认证检查: 检查 CSDN Cookie 中的 UserName
// 图片上传: 华为云 OBS (Object Storage Service) 直传
// 发布流程: Markdown + 图片 URL 替换 → POST /api/articles/draft/save
// ============================================================================
const ne = N("CSDN");
class zr extends D {
    constructor() {
    ...
    async checkAuth() {
        var e;
        try {
            const t = "/blog-console-api/v3/editor/getBaseInfo",
                r = await this.signRequest(t, "GET"),
            ...
            return ne.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
...
${e}`;
        ...
        const n = await this.hmacSha256(s, this.API_SECRET),
            i = {
                accept: "*/*",
                "x-ca-key": this.API_KEY,
                "x-ca-nonce": r,
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const s = e.html || "",
                n = "/blog-console-api/v3/mdeditor/saveArticle",
                i = await this.signRequest(n),
            ...
            const l = u.data.id,
                h = `https://editor.csdn.net/md?articleId=${l}`;
            return this.createResult(!0, {
                postId: l,
                postUrl: h,
                draftOnly: (t == null ? void 0 : t.draftOnly) ?? !0
    ...
    async uploadImageByUrl(e) {
        ...
        const r = await t.blob(),
            ...
            i = "/resource-api/v1/image/direct/upload/signature",
            a = await this.signRequest(i, "POST"),
        ...
        if (ne.debug("Upload signature response:", l), l.code !== 200 || !l.data) return ne.warn("Failed to get upload signature, using original URL"), {
            url: e
        };
        const h = l.data,
            d = h.customParam,
        ...
        const p = await (await this.runtime.fetch(h.host, {
            method: "POST",
            body: m
        })).json();
        return ne.debug("OBS upload response:", p), p.code !== 200 || !((y = p.data) != null && y.imageUrl) ? (ne.warn("OBS upload failed, using original URL"), {
            url: e
        }) : {
            url: p.data.imageUrl
        }
...
// ============================================================================
// 【微博适配器 - Wr (WeiboAdapter)】
// 平台: 微博头条文章 (card.weibo.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: GET card.weibo.com 验证登录态
// 图片上传（特殊流程）:
//   - 异步上传: POST 上传请求后轮询图片状态（最多 30 次，间隔 1s）
//   - Data URI 直传: 支持 base64 data URI 直接上传
//   - 自定义 processWeiboImages(): 处理 <figure> 和 <img> 混合情况
//   - reqId 生成: 每次请求生成唯一 ID 用于追踪
// 发布流程: HTML 内容 + 图片上传 → 创建/更新头条文章草稿
// ============================================================================
const L = N("Weibo");
class Wr extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            ...
            return L.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    async getUserConfig() {
        ...
        const r = (await (await this.runtime.fetch("https://card.weibo.com/article/v5/editor", {
            credentials: "include"
        })).text()).match(/config:\s*JSON\.parse\('(.+?)'\)/);
        if (!r) return L.error("Failed to find config in HTML"), null;
        try {
            const s = r[1].replace(/\\'/g, "'").replace(/\\\\/g, "\\"),
                n = JSON.parse(s);
    ...
    async uploadImageByUrl(e) {
        ...
        return {
            url: `https://wx3.sinaimg.cn/large/${s.pid}.jpg`,
            attrs: {
                "data-pid": s.pid
            }
        }
    ...
    async uploadDataUri(e) {
        ...
        return {
            url: `https://wx3.sinaimg.cn/large/${m}.jpg`,
            attrs: {
                "data-pid": m
            }
        }
    ...
    async processWeiboImages(e, t) {
        var m;
        const r = e,
            s = /<figure[^>]*>[\s\S]*?<img[^>]+src="([^"]+)"[^>]*>[\s\S]*?<\/figure>/gi,
            n = /<img[^>]+src="([^"]+)"[^>]*>/gi,
            i = [];
        let a;
        const u = new Set;
        for (;
            (a = s.exec(r)) !== null;) i.push({
            full: a[0],
            src: a[1],
            hasFigure: !0
        }), u.add(a[1]);
        for (;
            (a = n.exec(r)) !== null;) u.has(a[1]) || i.push({
            full: a[0],
            src: a[1],
            hasFigure: !1
        });
        for (const f of xe(r)) i.push({
            full: f.full,
            src: f.src,
            hasFigure: !1
        });
                ...
                if (!p.startsWith("data:")) {
                    ...
                    try {
                        ...
                        if (!_) {
                            ...
                            _ = {
                                pid: ((m = T.attrs) == null ? void 0 : m["data-pid"]) || "",
                                url: T.url
                            }, h.set(p, _)
                        }
                        let y;
    ...
    async waitForImageDone(e) {
        ...
        for (let n = 0; n < r; n++) {
            const i = this.generateReqId(),
                ...
                })).json()).data) == null ? void 0 : s[0],
                h = l == null ? void 0 : l.task_status_code;
...
// ============================================================================
// 【B站专栏适配器 - Xr (BilibiliAdapter)】
// 平台: 哔哩哔哩专栏 (bilbili.com/read)
// 能力: article, draft, image_upload
// 输出格式: HTML
// CSRF: 从 Cookie 中提取 bili_jct 作为 CSRF token
// 认证检查: GET api.bilibili.com/x/web-interface/nav 判断登录态
// 图片上传: multipart 上传到 bilibili 图片服务器（upcover 接口）
// 发布流程: HTML 内容 + 封面图 + 分类 → form POST 保存草稿
// ============================================================================
const le = N("Bilibili");
class Xr extends D {
    constructor() {
    ...
    async checkAuth() {
        ...
        try {
            ...
            return le.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async uploadImageByUrl(e) {
        ...
        return {
            url: a.data.url,
            attrs: {
                size: String(a.data.size)
            }
        }
    }
}
// ============================================================================
// 【百家号适配器 - Gr (BaijiahaoAdapter)】
// 平台: 百度百家号 (baijiahao.baidu.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: 从 HTML 页面提取 auth token（正则匹配）
// 图片上传: multipart 上传到百度图片服务器（uploadproxy 接口）
// JSONP 解析: 部分接口返回 JSONP 格式，需要提取 JSON 部分
// 发布流程: HTML 内容 + 图片 URL 替换 → POST 保存草稿
// ============================================================================
...
class Gr extends D {
    ...
    async checkAuth() {
        ...
        try {
            ...
            return ye.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async fetchAuthToken() {
        const r = (await (await this.runtime.fetch("https://baijiahao.baidu.com/builder/rc/edit", {
            credentials: "include"
        })).text()).match(/window\.__BJH__INIT__AUTH__\s*=\s*['"]([^'"]+)['"]/);
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const i = (await (await this.runtime.fetch("https://baijiahao.baidu.com/pcui/article/save?callback=bjhdraft", {
                ...
                })).text()).replace(/^bjhdraft\(/, "").replace(/\)$/, ""),
                a = JSON.parse(i);
            if (ye.debug("Save response:", a), a.errmsg !== "success" || !((h = a.ret) != null && h.article_id)) throw new Error(a.errmsg || "保存草稿失败");
            const u = a.ret.article_id,
                l = `https://baijiahao.baidu.com/builder/rc/edit?type=news&article_id=${u}`;
            return this.createResult(!0, {
                postId: u,
                postUrl: l,
                draftOnly: (t == null ? void 0 : t.draftOnly) ?? !0
...
// ============================================================================
// 【语雀适配器 - Jr (YuqueAdapter)】
// 平台: 语雀知识库 (yuque.com)
// 能力: article, draft, image_upload
// 输出格式: Markdown（通过 API 转换为 Lake 格式）
// CSRF: 从 Cookie 中提取 _yuque_session 作为认证 token
// 认证检查: GET yuque.com/api/v2/user 判断登录态
// 内容转换: Markdown 通过语雀 API 转换为 Lake 编辑器格式
// 图片上传: multipart 上传到语雀附件服务
// 发布流程: Markdown → Lake 格式转换 + 图片上传 → 保存文档草稿
// ============================================================================
const be = N("Yuque");
class Jr extends D {
    constructor() {
    ...
    async checkAuth() {
        ...
        try {
            ...
            return be.debug("checkAuth: not logged in -", t.message), {
                isAuthenticated: !1,
                error: t.message
            }
        }
...
// ============================================================================
// 【微信公众号适配器 - Yr (WeixinAdapter)】
// 平台: 微信公众号 (mp.weixin.qq.com)
// 能力: article, draft, image_upload
// 输出格式: HTML（内联 CSS 样式）
// 预处理特殊配置:
//   - LaTeX 公式处理（转为图片）
//   - 外链剥离（微信不允许外部链接，仅保留微信域名链接）
//   - 内联 CSS 样式注入（微信编辑器不支持 <style> 标签）
// 认证检查: 访问 mp.weixin.qq.com 提取 token 和 cookie
// 图片上传: filetransfer API 上传到微信素材库
// 错误码映射: 完整的微信 API 错误码到中文描述的映射（方便用户理解错误）
// 发布流程: HTML 内联样式 + 图片上传 → 保存为公众号草稿
// ============================================================================
const de = N("Weixin"),
    Kr = `
p {
...
class Yr extends D {
    ...
    async checkAuth() {
        try {
            const t = await (await this.runtime.fetch("https://mp.weixin.qq.com/", {
                ...
                r = t.match(/data:\s*\{[\s\S]*?t:\s*["']([^"']+)["']/);
            if (!r) return de.debug(" No token found"), {
                isAuthenticated: !1
            };
            const s = t.match(/ticket:\s*["']([^"']+)["']/),
                n = t.match(/user_name:\s*["']([^"']+)["']/),
                i = t.match(/nick_name:\s*["']([^"']+)["']/),
                a = t.match(/time:\s*["'](\d+)["']/),
            ...
            return de.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    async uploadImageByUrl(e) {
        ...
        const {
            ...
            ticket: l,
            svrTime: h
        } = this.weixinMeta, d = Date.now(), f = await (await this.runtime.fetch(`https://mp.weixin.qq.com/cgi-bin/filetransfer?action=upload_material&f=json&scene=8&writetype=doublewrite&groupid=1&ticket_id=${u}&ticket=${l}&svr_time=${h}&token=${a}&lang=zh_CN&seq=${d}&t=${Math.random()}`, {
            method: "POST",
            credentials: "include",
            body: s
        })).json();
...
// ============================================================================
// 【51CTO 适配器 - Vr (51CTOAdapter)】
// 平台: 51CTO 博客 (blog.51cto.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: GET 51cto.com 页面判断登录态
// 图片上传（链式流程）:
//   1. 请求上传签名（upload sign 接口）
//   2. 获取上传配置（upload config 接口）
//   3. 直传腾讯云 COS (Cloud Object Storage)
// ============================================================================
class Vr extends D {
    constructor() {
        super(...arguments);
    ...
    async checkAuth() {
        try {
            const t = await (await this.runtime.fetch("https://blog.51cto.com/blogger/publish", {
                    credentials: "include"
                })).text(),
            ...
            const s = r[1],
                n = r[2],
                i = s.split("/").filter(Boolean).pop() || "",
                a = t.match(/<meta\s+name="csrf-token"\s+content="([^"]+)"/);
            return a && (this.csrf = a[1]), {
                isAuthenticated: !0,
                userId: i,
                username: i,
                avatar: n
    ...
    async uploadToCOS(e, t, r) {
        ...
        const n = await this.runtime.fetch(e, {
            method: "POST",
            body: s
        });
    ...
    async uploadImageByUrl(e) {
        const r = await (await this.runtime.fetch(e)).blob(),
            ...
            u = await this.getUploadSign(),
            l = await this.getUploadConfig(u.sign, s, i);
        return {
            url: await this.uploadToCOS(l.url, l.fields, a)
        }
    }
    async publish(e) {
        const t = Date.now();
...
// ============================================================================
// 【慕课网适配器 - Qr (ImoocAdapter)】
// 平台: 慕课网手记 (imooc.com/article)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: JSONP 方式访问 imooc.com 接口判断登录态
// 图片上传: multipart 上传到慕课网图片服务器
// ============================================================================
}
class Qr extends D {
    constructor() {
    ...
    async checkAuth() {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            let t = await (await this.runtime.fetch("https://www.imooc.com/u/card", {
                credentials: "include"
            })).text();
...
// ============================================================================
// 【开源中国适配器 - Zr (OschinaAdapter)】
// 平台: 开源中国博客 (oschina.net/blog)
// 能力: article, draft, image_upload
// 输出格式: 自动检测（Markdown 或 HTML，根据平台编辑器决定）
// 认证检查: 访问 oschina.net 页面判断登录态
// 内容类型检测: 检查平台编辑器类型，选择 Markdown 或 HTML 输出
// ============================================================================
}
class Zr extends D {
    constructor() {
    ...
    async checkAuth() {
        var e, t, r;
        try {
            const n = await (await this.runtime.fetch("https://apiv1.oschina.net/oschinapi/user/myDetails", {
                credentials: "include"
            })).json();
    ...
    async uploadImageByUrl(e) {
        ...
        const r = await (await this.runtime.fetch(e)).blob(),
            s = this.getFilenameFromUrl(e) || "image",
            n = new FormData;
        n.append("file", r, s);
...
// ============================================================================
// 【SegmentFault 适配器 - es (SegmentFaultAdapter)】
// 平台: SegmentFault 思否 (segmentfault.com)
// 能力: article, draft, image_upload
// 输出格式: Markdown
// 认证检查: 从 HTML 页面提取 session token（正则匹配）
// 图片上传: multipart 上传到 SegmentFault 图片服务
// 响应解析: 特殊数组格式响应（[errCode, data]）
// ============================================================================
}
class es extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            const t = await (await this.runtime.fetch("https://segmentfault.com/user/settings", {
                    credentials: "include"
                })).text(),
                r = t.match(/href="\/u\/([^"]+)"/);
            if (!r) return {
            ...
            const s = r[1],
                n = t.match(/src="(https:\/\/avatar-static\.segmentfault\.com\/[^"]+)"/),
                i = n ? n[1] : void 0;
            return {
                isAuthenticated: !0,
                userId: s,
    ...
    async getSessionToken() {
        var u, l;
        const t = await (await this.runtime.fetch("https://segmentfault.com/write", {
                credentials: "include"
            })).text(),
            r = t.match(/serverData":\s*\{\s*"Token"\s*:\s*"([^"]+)"/);
        if (r) return r[1];
        const s = "window.g_initialProps = ",
            n = t.indexOf(s);
...
// ============================================================================
// 【博客园适配器 - ts (CnblogsAdapter)】
// 平台: 博客园 (cnblogs.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// XSRF: 从 Cookie 中提取 XSRF-TOKEN
// 认证检查: GET cnblogs.com/api 判断登录态
// 图片上传: multipart 上传到博客园图片服务
// 发布流程: 完整的 API POST 请求体（包含标题、内容、分类、标签等）
// ============================================================================
const z = N("Cnblogs");
class ts extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            ...
            return z.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const l = String(u.id),
                h = `https://i.cnblogs.com/articles/edit;postId=${l}`;
    ...
    async uploadImageByUrl(e) {
        ...
        return z.info("Image uploaded:", u), {
            url: u
        }
    }
}
// ============================================================================
// 【ZIP 下载适配器 - rs (ZipDownloadAdapter)】
// 功能: 将文章打包为 ZIP 文件下载（非发布平台）
// 能力: article, zip_download
// 实现:
//   - 使用 JSZip 库创建 ZIP 包
//   - 并行下载图片（4 个 worker 并发）
//   - 文件名清理（移除非法字符）
//   - 生成 Markdown 文件 + images/ 目录
// ============================================================================
...
class rs extends D {
    ...
    async publish(e, t) {
        try {
            ...
            if (!n.trim()) return this.createResult(!1, {
                error: "文章内容为空"
            });
...
${n}`);
            ...
            if (!this.runtime.downloads) return this.createResult(!1, {
                error: "当前环境不支持下载功能"
            });
            ...
            return he.error("ZIP download failed:", r), this.createResult(!1, {
                error: r.message
            })
        }
    }
    async processImagesForZip(e, t, r) {
        ...
        const d = async f => {
            try {
                const p = await this.runtime.fetch(f, {
                    credentials: "omit"
                });
                if (!p.ok) {
                ...
                const w = await p.blob(),
                    _ = this.getImageExtension(f, w.type);
                a++;
        ...
        return {
            processedMarkdown: i,
            imageCount: Array.from(l.values()).filter(Boolean).length
        }
    }
...
// ============================================================================
// 【东方财富适配器 - ss (EastmoneyAdapter)】
// 平台: 东方财富号 (eastmoney.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 设备ID: 持久化到 storage（首次生成后复用）
// 认证检查: Cookie 中提取 auth token
// 发布流程: 草稿创建/更新 API（区分新建和编辑模式）
// ============================================================================
...
class ss extends D {
    ...
    async checkAuth() {
        ...
        try {
            ...
            return _e.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async buildParm(e) {
        ...
        return [{
            ...
            text: encodeURIComponent(e.text)
        }, {
            columns: "2"
        }, {
            cover: ""
    ...
    async updateDraft(e, t, r) {
        const s = await this.buildParm({
            draftid: e,
            title: t,
            text: `<div class="xeditor_content cfh_web">${r}</div>`
        });
    ...
    async uploadImageByUrl(e) {
        ...
        if (r.code === 200 && ((s = r.data) != null && s.url)) return {
            url: r.data.url
        };
    ...
    async uploadImageBlob(e) {
        ...
        if (i.code === 200 && ((a = i.data) != null && a.url)) return {
            url: i.data.url
        };
...
// ============================================================================
// 【大鱼号适配器 - as (DaYuAdapter)】
// 平台: UC 大鱼号 (mp.dayu.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: 从 HTML 页面提取 globalConfig（正则匹配，有降级策略）
// 图片上传: multipart 上传 + 上传进度追踪
// ============================================================================
const He = N("DaYu");
class as extends D {
    constructor() {
    ...
    async checkAuth() {
        ...
        try {
            ...
            const a = s.substring(i + n.length, s.indexOf("var G = {", i)),
                u = this.parseGlobalConfig(a);
            return !u || !u.utoken ? {
                isAuthenticated: !1
            } : (this.cacheMeta = {
                utoken: u.utoken,
            ...
            return He.debug("checkAuth: not logged in -", r), {
                isAuthenticated: !1,
                error: r.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const s = this.uploadedImages.length > 0 ? this.uploadedImages[0].url : "",
            ...
            const u = a.data._id,
                l = `https://mp.dayu.com/dashboard/article/write?draft_id=${u}`;
            return this.createResult(!0, {
                postId: u,
                postUrl: l,
                draftOnly: (t == null ? void 0 : t.draftOnly) ?? !0
    ...
    async uploadImageByUrl(e) {
        ...
        const u = await (await this.runtime.fetch(s, {
            method: "POST",
            credentials: "include",
            body: n
        })).json();
        ...
        const l = {
            org_url: u.data.imgInfo.org_url,
            url: u.data.imgInfo.url
        };
        return this.uploadedImages.push(l), {
            url: l.url
        }
    }
}
...
// ============================================================================
// 【抖音适配器 - is (DouyinAdapter)】
// 平台: 抖音创作者 (creator.douyin.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 执行方式: 通过 Chrome tabs.executeScript 在抖音页面注入代码
// STS 凭证: 获取临时安全令牌（STS Token）用于 ImageX/TOS 上传
// 图片上传: 字节跳动 ImageX + TOS 上传（与掘金共享签名逻辑）
// 内容限制: 正文最多 8000 字符，超出自动截断
// 发布流程: 注入 content script → 图片上传 → 通过页面 API 保存草稿
// ============================================================================
class is extends D {
    constructor() {
        super(...arguments);
    ...
    async checkAuth() {
        try {
            ...
            return V.debug("checkAuth: not logged in -", e), {
                isAuthenticated: !1,
                error: e.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            r = await this.processImages(r, async d => {
                ...
                return m.imageInfo && s.push(m.imageInfo), {
                    url: m.storeUri
                }
            }, {
    ...
    async uploadImageFull(e) {
        var h, d;
        let t;
        if (e.startsWith("data:")) t = await fetch(e).then(m => m.blob());
        else {
            const m = await this.runtime.fetch(e, {
                method: "GET"
            });
            if (!m.ok) return V.warn("Failed to download image:", m.status), {
                storeUri: e
            };
            t = await m.blob()
        }
        const r = await this.getSTSCredentials(),
            s = await this.applyImageUpload(r),
        ...
        const a = (d = (await this.commitImageUpload(r, s.SessionKey)).PluginResult) == null ? void 0 : d[0],
            u = await this.getImagePreviewUrl(n),
            l = {
                key: n,
                value: {
                    url: u,
        ...
        return V.debug("Image uploaded:", n, `(${l.value.width}x${l.value.height})`), {
            storeUri: n,
            imageInfo: l
        }
    }
    async uploadImageByUrl(e) {
        try {
            return {
                url: (await this.uploadImageFull(e)).storeUri
            }
        } catch (t) {
            return V.warn("Failed to upload image:", e, t), {
                url: e
            }
        }
    }
...
const cs = Object.freeze(Object.defineProperty({
    ...
    pe = N("Jianshu");
// ============================================================================
// 【简书适配器 - us (JianshuAdapter)】
// 平台: 简书 (jianshu.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: GET jianshu.com/authorizations 判断登录态
// 图片上传: 七牛云 OSS 直传（获取上传 token → 直传七牛 CDN）
// 笔记本管理: 获取用户笔记本列表，检测首选笔记类型
// 发布流程: HTML 内容 + 图片 URL 替换 → 创建/更新简书草稿
// ============================================================================
class us extends D {
    constructor() {
        super(...arguments);
    ...
    async checkAuth() {
        ...
        try {
            ...
            return pe.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async uploadImageByUrl(e) {
        ...
        try {
            ...
            const {
                token: a,
                key: u
            } = await this.getUploadToken(i), l = new FormData;
            l.append("token", a), l.append("key", u), l.append("file", s, i), l.append("x:protocol", "https");
            const d = await (await fetch("https://upload.qiniup.com/", {
                method: "POST",
                body: l
            })).json();
            if (pe.debug("Image upload response:", d), d.url) return {
                url: d.url
            };
            throw new Error("图片上传失败")
        } catch (r) {
            return pe.warn("Failed to upload image:", e, r), {
                url: e
            }
        }
    }
...
const ls = Object.freeze(Object.defineProperty({
    ...
    ee = N("Netease");
// ============================================================================
// 【网易号适配器 - ds (NeteaseAdapter)】
// 平台: 网易号 (mp.163.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 执行方式: 通过 Chrome tabs.executeScript 注入代码获取 ursToken
//   - 调用页面中的 neg.getToken() 获取认证令牌
// 表格处理: 将 HTML 表格转换为纯文本格式（网易号不支持表格）
// 图片上传: multipart 上传到网易图片服务
// ============================================================================
class ds extends D {
    constructor() {
        super(...arguments);
    ...
    async checkAuth() {
        ...
        try {
            ...
            return ee.debug("checkAuth response:", t), t.code !== 1 || !((e = t.data) != null && e.tid) ? {
                isAuthenticated: !1
            ...
            return ee.debug("checkAuth: not logged in -", t), {
                isAuthenticated: !1,
                error: t.message
            }
        }
    ...
    async publish(e, t) {
        return this.withHeaderRules(this.HEADER_RULES, async () => {
            ...
            const n = this.accountInfo.tid,
                i = this.accountInfo.realUserId || "",
                a = Date.now(),
    ...
    async fetchUrsToken() {
        ...
        const t = await this.runtime.tabs.executeScript(e, async () => {
            try {
                const r = window.neg;
                if (!(r != null && r.getToken)) return {
                    success: !1,
                    error: "neg.getToken not available"
                };
                const s = await r.getToken();
                return s.code === 200 && s.token ? {
                    success: !0,
                    token: s.token
                } : {
                    success: !1,
...
// ============================================================================
// 【什么值得买适配器 - gs (SmzdmAdapter)】
// 平台: 什么值得买 (post.smzdm.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// WAF 防护: 检测并处理 WAF 验证挑战（最多重试 5 次）
// 认证检查: GET post.smzdm.com/api 判断登录态
// 发布流程: 文章创建 API（含 WAF 重试逻辑）
// ============================================================================
class gs extends D {
    constructor() {
        super(...arguments);
    ...
    async fetchWithRetry(e, t, r = 5) {
        ...
        for (let i = 1; i <= r; i++) {
            const a = await this.runtime.fetch(e, n),
                l = await a.clone().text();
            if (!ps(l)) return new Response(l, {
                status: a.status,
    ...
    async checkAuth() {
        try {
            const t = await (await this.fetchWithRetry("https://post.smzdm.com/tougao/", {
                credentials: "include"
            })).text();
            if (!t.includes("release-new")) return {
                isAuthenticated: !1,
                error: "未登录"
            };
    ...
    async uploadImageByUrl(e) {
        ...
        return qe.debug(`Image uploaded: ${i.data.url}`), {
            url: i.data.url
        }
    }
    async publish(e) {
...
const fs = Object.freeze(Object.defineProperty({
    ...
    value: "Module"
// ============================================================================
// 【搜狐焦点适配器 - ws (SohuFocusAdapter)】
// 平台: 搜狐焦点 (focus.cn)
// 能力: article, draft
// 输出格式: HTML
// 简单适配器: 直接 HTML 发布，无特殊图片上传逻辑
// ============================================================================
}));
class ws extends D {
    constructor() {
    ...
    async checkAuth() {
        var e;
        try {
            const r = await (await this.runtime.fetch("https://mp-fe-pc.focus.cn/user/status", {
                credentials: "include"
            })).json();
    ...
    async uploadImageByUrl(e) {
        ...
        return {
            url: `https://t-img.51f.com/sh740wsh${i.data}`
        }
    }
    async publish(e) {
        var r;
...
const ys = Object.freeze(Object.defineProperty({
    ...
    oe = N("Toutiao");
// ============================================================================
// 【头条号适配器 - bs (ToutiaoAdapter)】
// 平台: 今日头条创作者 (mp.toutiao.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 执行方式: 通过 content script 注入页面执行发布操作
// CSRF: 从 Cookie 提取 csrf token
// 图片处理: 图片包裹在 pgc-img div 中（头条号编辑器要求）
// 发布流程: 注入脚本 → 图片处理 → 通过页面 API 保存草稿
// ============================================================================
class bs extends D {
    constructor() {
        super(...arguments);
    ...
    async checkAuth() {
        ...
        try {
            ...
            return oe.debug("checkAuth: not logged in -", r), {
                isAuthenticated: !1,
                error: r.message
            }
        }
...
`), r = await this.processImages(r, m => this.uploadImageByUrl(m), {
            ...
            const s = "[]",
                ...
                i = `${Date.now()}_${Math.random().toString().slice(2,18)}`,
                a = new URLSearchParams;
            a.append("pgc_id", "0"), a.append("source", "29"), a.append("extra", n), a.append("content", r), a.append("title", e.title), a.append("search_creation_info", JSON.stringify({
                ...
                abstract: "",
                clue_id: ""
            })), a.append("title_id", i), a.append("mp_editor_stat", "{}"), a.append("is_refute_rumor", "0"), a.append("save", "0"), a.append("timer_status", "0"), a.append("timer_time", ""), a.append("educluecard", ""), a.append("draft_form_data", JSON.stringify({
                coverType: 3
            })), a.append("pgc_feed_covers", s), a.append("article_ad_type", "3"), a.append("is_fans_article", "0"), a.append("govern_forward", "0"), a.append("praise", "0"), a.append("disable_praise", "0"), a.append("tree_plan_article", "0"), a.append("activity_tag", "0"), a.append("trends_writing_tag", "0"), a.append("claim_exclusive", "0");
            ...
            const l = u.data.pgc_id,
                h = `https://mp.toutiao.com/profile_v4/graphic/publish?pgc_id=${l}`;
            return this.createResult(!0, {
                postId: l,
                postUrl: h,
                draftOnly: (t == null ? void 0 : t.draftOnly) ?? !0
...
const _s = Object.freeze(Object.defineProperty({
    ...
    Ss = /https:\/\/abs\.twimg\.com\/responsive-web\/client-web(?:-legacy)?\/[A-Za-z0-9.-]+\.js/g,
    Ts = [{
...
// ============================================================================
// 【X/Twitter 适配器 - As (TwitterAdapter)】
// 平台: X (Twitter)
// 能力: article, draft, image_upload, long_tweet
// 输出格式: Draft.js JSON（X 的长文本编辑器使用）
// API 交互:
//   - GraphQL API（需要动态发现 Query ID）
//   - 从 X 的 JS bundle 文件中正则匹配提取 CreateTweet/EditTweet 的 queryId
//   - Bearer Token 固定值（ks 常量）
// 图片上传（三步流程）:
//   1. INIT: 初始化上传（指定文件大小和类型）
//   2. APPEND: 分块上传二进制数据
//   3. FINALIZE: 完成上传并获取 media_id
// 内容格式: Draft.js block 结构，支持 LaTeX/divider/code block 等富文本
// XPForwardedForSDK: 集成 X 的 SDK 用于请求签名
// ============================================================================
class As extends D {
    constructor() {
        super(...arguments);
    ...
    async getQueryId(e) {
        ...
        try {
            ...
            const r = await this.getQueryIdCache(),
                s = Be[e],
                n = r == null ? void 0 : r.ids[s];
    ...
    async discoverBundles() {
        ...
        for (const t of Is) try {
            ...
            const n = (await (await this.runtime.fetch(t, {
                credentials: "include"
            })).text()).matchAll(Ss);
    ...
    extractOperationsFromJs(e, t, r) {
        for (const s of Ts) {
            ...
            let n;
            for (;
                (n = s.regex.exec(e)) !== null;) {
                const i = n[s.operationGroup],
                    a = n[s.queryIdGroup];
    ...
    async graphqlRequest(e, t, r) {
        const s = async () => {
            const i = await this.getQueryId(e),
                a = Be[e],
                u = await this.getCommonHeaders(),
    ...
    async getCsrfToken() {
        ...
        if (this.runtime.tabs) try {
            const r = await this.ensureXTab(),
                s = await this.runtime.tabs.executeScript(r, async () => {
                    try {
            ...
            const t = await this.runtime.tabs.executeScript(e, async () => {
                try {
                    ...
                    return !r || typeof r.getForwardedForStr != "function" ? {
                        ...
                        success: !0,
                        data: await r.getForwardedForStr()
                    }
                } catch (r) {
                    return {
                        success: !1,
    ...
    async setupHeaderRules() {
        this.headerRuleIds.length > 0 || await this.addHeaderRules([{
            ...
            headers: {
                Origin: "https://x.com",
                Referer: "https://x.com/compose/articles"
            },
            resourceTypes: ["xmlhttprequest"]
        }, {
    ...
    async checkAuth() {
        ...
        try {
            ...
            const s = await (await this.runtime.fetch("https://x.com/home", {
                credentials: "include"
            })).text();
            R.debug("Fetched x.com/home, length:", s.length);
            const n = s.indexOf("window.__INITIAL_STATE__=");
            if (n === -1) return R.debug("Failed to find __INITIAL_STATE__"), await this.clearHeaderRules(), {
                isAuthenticated: !1
            };
            const i = s.indexOf("{", n);
            if (i === -1) return R.debug("Failed to find JSON start"), await this.clearHeaderRules(), {
                isAuthenticated: !1
            };
            let a = 0,
                u = i;
            for (let w = i; w < s.length; w++)
                if (s[w] === "{" ? a++ : s[w] === "}" && a--, a === 0) {
                    u = w + 1;
                    break
                } const l = s.slice(i, u);
            ...
            try {
                ...
                return R.debug("Failed to parse __INITIAL_STATE__:", w), await this.clearHeaderRules(), {
                    isAuthenticated: !1
                }
            }
            const d = (t = (e = h.entities) == null ? void 0 : e.users) == null ? void 0 : t.entities;
            if (!d) return R.debug("No user entities found in state"), await this.clearHeaderRules(), {
                isAuthenticated: !1
            };
            const m = Object.keys(d);
            if (m.length === 0) return R.debug("No user IDs found"), await this.clearHeaderRules(), {
                isAuthenticated: !1
            };
            const f = m[0],
                p = d[f];
    ...
    async publish(e, t) {
        ...
        try {
            ...
            return await this.clearHeaderRules(), R.error("Publish failed:", r), this.createResult(!1, {
                error: r.message
            })
        }
    }
    async createDraft() {
        var r, s, n, i, a;
        const e = await this.graphqlRequest("createDraft", {
            content_state: {
                blocks: [],
                entity_map: []
            },
    ...
    async updateTitle(e, t) {
        var s;
        const r = await this.graphqlRequest("updateTitle", {
            articleEntityId: e,
            title: t
        });
    ...
    async updateContent(e, t) {
        var s;
        const r = await this.graphqlRequest("updateContent", {
            content_state: t,
            article_entity: e
        });
        if ((s = r.errors) != null && s.length) throw new Error(r.errors[0].message)
    ...
    async uploadImageByUrl(e) {
        ...
        return R.debug("Media FINALIZE complete"), {
            url: u,
            attrs: {
                "media-id": u
            }
...
`;
        ...
        for (const [y, T] of Object.entries(u.entityMap)) {
            const P = parseInt(y, 10),
                A = T.type.toUpperCase();
            if (A === "IMAGE") {
                const S = ((p = T.data) == null ? void 0 : p.src) || "",
                    v = i.get(S);
                if (v) {
                    const U = f++;
                    m.set(P, U), h.push({
                ...
                m.set(P, S), h.push({
                    ...
                    value: {
                        ...
                        mutability: "Mutable",
                        data: {
                            url: ((w = T.data) == null ? void 0 : w.url) || ((_ = T.data) == null ? void 0 : _.href) || ""
                        }
                    }
                })
            }
        ...
        for (const y of u.blocks) {
            const T = y.key || this.generateBlockKey(),
                P = y.text.match(/^\[XIMG_PLACEHOLDER_(\d+)\]$/);
            if (P) {
            ...
            let A = "unstyled";
            const S = y.type;
            if (S === "header-one" || S === "header-two") A = S;
            else if (S === "header-three" || S === "header-four" || S === "header-five" || S === "header-six") A = "header-two";
            else if (S === "blockquote") A = "blockquote";
            else if (S === "unordered-list-item") A = "unordered-list-item";
            else if (S === "ordered-list-item") A = "ordered-list-item";
            else if (S === "code-block") {
                const k = f++;
                h.push({
                    key: String(k),
                    value: {
                ...
                const k = y.text.trim().slice(2, -2).trim(),
                    O = f++;
                h.push({
                    key: String(O),
                    value: {
            ...
            const U = [];
            if (y.inlineStyleRanges)
                for (const k of y.inlineStyleRanges) {
                    const O = this.convertInlineStyle(k.style);
                    O && U.push({
                        style: O,
                        offset: k.offset,
                        length: k.length
...
// ============================================================================
// 【小红书 Markdown → ProseMirror 内容转换器】
// Bt(markdown): Markdown AST 解析器
//   - 逐行解析 Markdown 文本
//   - 支持的块级元素: heading、paragraph、code block、blockquote、list、hr
//   - 支持的内联元素: bold、italic、code、link、image
//   - 输出: 中间 AST 节点数组
// Ce(nodes): AST → ProseMirror 文档转换器
//   - 将 Bt() 输出的 AST 转换为 ProseMirror JSON 格式
//   - 小红书编辑器基于 ProseMirror，需要特定的文档结构
// 平台能力配置:
//   - xiaohongshu: 小红书专用（支持 figure、divider）
//   - juejin: 掘金格式（支持 code block 语言标记）
//   - zhihu: 知乎格式（支持 figure 包裹图片）
// ============================================================================
function Bt(c) {
    const o = c.split(`
`),
...
async function Ds(c, o, e) {
    ...
    if (Wt(c, t), t.uploadImage && t.imageQueue.length > 0) {
        let n = 0;
        for (const i of t.imageQueue)
            if (!t.uploadedImages.has(i.src)) try {
...
function Wt(c, o) {
    ...
    if ("children" in c && Array.isArray(c.children))
        for (const e of c.children) Wt(e, o)
}
...
function js(c, o) {
    ...
    switch (c.type) {
        ...
        case "thematicBreak":
            return e.supportHorizontalRule ? [{
                type: "horizontalRule"
            }] : [];
        case "code":
...
function Ns(c, o) {
    ...
    return [{
        type: "heading",
        attrs: {
            level: Math.min(c.depth, e.maxHeadingLevel)
        },
        content: X(c.children, o)
    }]
}
...
function qs(c, o) {
    ...
    for (const r of c.children)
        ...
        if (e.supportNestedList) t.push(...Xt(r, o));
        else
            for (const s of r.children)
                if (s.type === "listItem")
                    for (const n of s.children) n.type === "paragraph" && t.push({
                        type: "paragraph",
                        content: [{
                            type: "text",
                            text: "  • "
...
function Gt(c, o) {
    const e = o.uploadedImages.get(c.url),
        t = 410;
    if (e) {
        const r = t,
            s = e.width > 0 ? Math.round(t * e.height / e.width) : 0;
        return [{
            type: "image",
            attrs: {
                imgs: [{
...
const te = N("Xiaohongshu"),
    ze = "https://creator.xiaohongshu.com/publish/publish?from=menu&target=article",
    Gs = "https://creator.xiaohongshu.com",
    At = 1e4;
// ============================================================================
// 【小红书适配器 - Js (XiaohongshuAdapter)】
// 平台: 小红书创作者 (creator.xiaohongshu.com)
// 能力: article, draft, image_upload
// 输出格式: Markdown → ProseMirror JSON（通过 Bt/Ce 转换）
// 内容限制: 正文最多 10000 字符
// 执行方式: 通过 Chrome tabs.executeScript 在小红书页面注入代码
// IndexedDB: 直接操作小红书的 IndexedDB 存储草稿数据
// 图片上传:
//   - 腾讯云 COS 直传
//   - 需要 X-s/X-t 请求签名（小红书反爬机制）
//   - 签名算法在注入的 content script 中执行
// 发布流程: 打开小红书编辑器 → 注入草稿到 IndexedDB → 触发页面保存
// ============================================================================
class Js extends D {
    constructor() {
        super(...arguments);
    ...
    async publish(e, t) {
        ...
        try {
            ...
            const r = e.markdown || "",
                s = this.getPlainTextLength(r);
            if (s > At) throw new Error(`文章字数超出小红书限制：当前 ${s} 字，最多 ${At} 字`);
            if (!this.runtime.tabs) throw new Error("小红书发布需要浏览器 tabs API 支持");
            const n = await this.ensureXHSTab(),
                i = await Xs(r, async l => {
                    var d, m, f;
                ...
                }, t == null ? void 0 : t.onImageProgress),
                a = this.generateUUID();
                            ...
                            _.onerror = y => {
                                ...
                                try {
                                    const y = _.result,
                                        ...
                                        P = Array.from(y.objectStoreNames);
                                    if (!y.objectStoreNames.contains("article-draft")) {
                                        y.close(), w({
                                            success: !1,
                                            error: `article-draft store not found. URL: ${T}, stores: ${P.join(", ")}`
                                        });
                                        return
                                    }
                                    const A = y.transaction(["article-draft"], "readwrite"),
                                        S = A.objectStore("article-draft"),
                                        v = {
                                            content: {
                                                contextStore: {
                                                    liveContext: {
                                    ...
                                    U.onsuccess = () => {
                                        y.close(), w({
                                            success: !0,
                                            error: `saved with uid: ${m}`
                                        })
                                    }, U.onerror = () => {
                                        var k;
                                        y.close(), w({
                                            success: !1,
                                            error: "put error: " + ((k = U.error) == null ? void 0 : k.message)
                                        })
                                    }, A.onerror = () => {
                                        var k;
                                        y.close(), w({
            ...
            return await this.clearHeaderRules(), te.error("Publish failed:", r), this.createResult(!1, {
                error: r.message
            })
        }
    }
    async uploadImageByUrl(e) {
        ...
        const a = await this.runtime.tabs.executeScript(t, async (l, h) => {
            ...
            try {
                ...
                const P = T.data.uploadTempPermits.find(I => I.uploadAddr === "ros-upload.xiaohongshu.com") || T.data.uploadTempPermits[0],
                    A = (f = P.fileIds) == null ? void 0 : f[0];
                if (!A) return {
                    success: !1,
                    error: "获取 fileId 失败"
                };
                const S = atob(l),
                    v = new Uint8Array(S.length);
                for (let I = 0; I < S.length; I++) v[I] = S.charCodeAt(I);
                const U = new Blob([v], {
                        type: h
                    }),
                    k = `https://${P.uploadAddr}/${A}`,
                    O = await fetch(k, {
                        method: "PUT",
                        headers: {
                ...
                if (!O.ok) return {
                    success: !1,
                    error: `上传失败: ${O.status} ${O.statusText}`
                };
                const b = O.headers.get("x-ros-preview-url");
                return {
                    success: !0,
                    fileId: A,
                    previewUrl: b
...
const Ks = Object.freeze(Object.defineProperty({
    ...
    Ue = N("Yidian");
// ============================================================================
// 【一点资讯适配器 - Ys (YidianAdapter)】
// 平台: 一点资讯 (yidianzixun.com)
// 能力: article, draft, image_upload
// 输出格式: HTML
// 认证检查: 从页面提取 mpCode（媒体人代码）
// 图片上传（双策略）:
//   1. 优先尝试 URL 上传（传入图片原始 URL）
//   2. 失败后降级为 multipart 上传
// 发布流程: HTML 内容 + 图片 URL 替换 → POST 保存草稿
// ============================================================================
class Ys extends D {
    constructor() {
    ...
    async checkAuth() {
        try {
            const r = (await (await this.runtime.fetch("https://mp.yidianzixun.com", {
                credentials: "include"
            })).text()).match(/<script id="__val_"[^>]*>([\s\S]*?)<\/script>/);
            if (!r) return {
                isAuthenticated: !1,
                error: "未找到用户数据"
            };
            const s = r[1],
                n = s.match(/window\.mpcode\s*=\s*['"]([a-f0-9]+)['"]/);
    ...
    async uploadImageByUrl(e) {
        ...
        try {
            const d = this.getHeaders(),
                ...
                p = await (await this.runtime.fetch(m, {
                    credentials: "include",
                    headers: d
                })).json();
            if (p.status === "success" && p.inner_addr) return Ue.debug(`Image uploaded via URL: ${p.inner_addr}`), {
                url: p.inner_addr
            }
        } catch (d) {
        ...
        return Ue.debug(`Image uploaded via multipart: ${l.url}`), {
            url: l.url
        }
    }
    async publish(e) {
...
const Vs = Object.freeze(Object.defineProperty({
    ...
    Qs = 30 * 1e3;
// ============================================================================
// 【Chrome 扩展运行时封装 - Zs (ExtensionRuntime)】
// 封装所有 Chrome Extension API 调用，为适配器提供统一的运行时接口：
//   Cookie 管理:
//     - getCookies(domain): 获取指定域名的 Cookie
//     - setCookie(url, name, value): 设置 Cookie
//     - removeCookie(url, name): 删除 Cookie
//   存储管理:
//     - getStorage(keys): 读取 chrome.storage.local
//     - setStorage(data): 写入 chrome.storage.local
//     - getSession(keys): 读取 chrome.storage.session
//     - setSession(data): 写入 chrome.storage.session
//   请求头注入:
//     - addHeaderRules(rules): 使用 declarativeNetRequest 添加请求头规则
//     - removeHeaderRules(ids): 移除请求头规则
//   标签页管理:
//     - queryTabs(query): 查询标签页
//     - createTab(url): 创建新标签页
//     - waitForTab(tabId): 等待标签页加载完成
//     - executeScript(tabId, code): 在标签页注入代码（MAIN world）
//   网络请求:
//     - fetch(url, options): 封装 fetch，默认 30s 超时
//   DOM 解析:
//     - parseDOM(html): 使用 DOMParser 解析 HTML
//   下载:
//     - download(blob, filename): 触发文件下载
// ============================================================================
class Zs {
    constructor(o) {
        g(this, "config");
        ...
        g(this, "headerRules", {
            add: async o => {
                const e = this.ruleIdBase + this.ruleIdCounter++,
                    t = `rule_${e}`;
    ...
    async fetch(o, e) {
        var n;
        const t = ((n = this.config) == null ? void 0 : n.timeout) ?? Qs,
...
// ============================================================================
// 【适配器注册初始化】
// ra(): 从私有适配器映射表 ta 中提取所有以 "Adapter" 结尾的导出
//   - 遍历 ta 中的所有模块，实例化并验证适配器类
// sa: 公开适配器数组（知乎、掘金、微博等 17 个公开平台）
// aa: 合并公开 + 私有适配器，统一转换为 { meta, factory, preprocessConfig } 格式
// Me(): 初始化函数（幂等），首次调用时：
//   1. 设置运行时实例到适配器注册表
//   2. 逐个注册所有适配器
//   3. 设置 Et 标志位防止重复初始化
...
function ra() {
    const c = [];
    for (const o of Object.values(ta))
        for (const [e, t] of Object.entries(o))
            if (typeof t == "function" && e.endsWith("Adapter")) try {
                const r = new t;
                r && r.meta && c.push(t)
            } catch {}
    return c
}
const sa = [Hr, Br, Wr, Xr, Gr, zr, Jr, Mr, Nr, jr, Yr, Fr, Vr, Qr, Zr, es, ts, rs, ss, ...ra()],
    aa = sa.map(c => {
        const o = new c;
        return {
            meta: o.meta,
            factory: () => new c,
            preprocessConfig: o.preprocessConfig
        }
    }),
    ge = Te("WechatSync"),
    na = ea();
...
async function Jt(c) {
    ...
    try {
        return await dt(o.checkAuth(), Qe, `认证检查超时（${Qe/1e3}秒）`)
    } catch (e) {
        return {
            isAuthenticated: !1,
            error: e.message
...
// 【认证检查系统 - Ne()】
// 批量检查所有平台的登录状态：
//   - 并行检查，最多 5 个平台同时（Promise.all 批处理）
//   - TTL 缓存策略:
//     * 已认证: 5 分钟内不重新检查
//     * 未认证: 30 秒内不重新检查
//   - 强制刷新: 参数 force=true 时忽略缓存
//   - 返回: Map<platformId, { isAuthenticated, user?, error? }>
// la(): 单平台发布函数
//   - 超时控制: 10 分钟超时（大型文章含图片上传可能很慢）
//   - 调用 adapter.publish(article) 执行实际发布
// ============================================================================
async function Ne(c = !1) {
    await Me();
    const o = fe.getAllMeta(),
        e = await ca(),
        t = Date.now(),
        r = [],
        s = [];
    ...
    for (const i of o) {
        const a = e[i.id],
            u = a != null && a.isAuthenticated ? oa : ia;
    ...
    if (s.length > 0) {
        ...
        for (let i = 0; i < s.length; i += vt) {
            const a = s.slice(i, i + vt),
                u = await Promise.all(a.map(async l => {
                    try {
                        const h = await fe.get(l.id);
                        if (h) {
                            ge.debug(` Checking auth for ${l.id}...`);
                            const d = await dt(h.checkAuth(), Qe, `认证检查超时（${Qe/1e3}秒）`);
                            return ge.debug(` ${l.id} auth result:`, d), or(l.id, d.isAuthenticated).catch(() => {}), e[l.id] = {
    ...
    return await ua(e), r.some(i => i.isAuthenticated) && De("first_platform_login").catch(() => {}), r
}
...
// ============================================================================
// 【同步编排器 - ot() (Sync Orchestrator)】
// 多平台并行发布的总调度器：
//   - AbortController: 支持取消正在进行的同步（da() 函数触发 abort）
//   - 并行批次: 每批最多 3 个平台同时发布
//   - 进度回调:
//     * onResult(platformId, result): 单个平台完成回调
//     * onImageProgress(platformId, current, total): 图片上传进度
//     * onDetailProgress(platformId, stage, message): 详细阶段信息
//   - 遥测事件: 记录同步耗时、成功/失败率
//   - 超时保护: 单个平台 10 分钟超时
//   - 来源追踪: 参数 source 区分来自 popup 还是 MCP
// ============================================================================
async function ot(c, o, e, t = "popup") {
    ...
    const r = Se.signal,
        s = [],
        n = Date.now();
    ir(t, c).catch(() => {}), cr().catch(() => {}), ur(c).catch(() => {}), De("first_sync_attempt").catch(() => {});
    const i = d => {
            const m = Pe().find(f => f.id === d);
        ...
        a = async d => {
            ...
            const f = Date.now(),
                p = e != null && e.onImageProgress || e != null && e.onDetailProgress ? (S, v, U) => {
                    var k, O, b;
                    (k = e == null ? void 0 : e.onImageProgress) == null || k.call(e, S, v, U), (O = e == null ? void 0 : e.onDetailProgress) == null || O.call(e, {
                        platform: d,
                        platformName: m,
                    ...
                    }), v === U && U > 0 && ((b = e == null ? void 0 : e.onDetailProgress) == null || b.call(e, {
                        platform: d,
                        platformName: m,
                        stage: "saving"
                    }))
                } : void 0,
                w = await la(d, o, void 0, p);
            (P = e == null ? void 0 : e.onDetailProgress) == null || P.call(e, {
                platform: d,
                platformName: m,
    ...
    for (let d = 0; d < c.length; d += Ut) {
        ...
        const m = c.slice(d, d + Ut),
            f = await Promise.all(m.map(a));
        s.push(...f)
    }
    Se = null;
    const u = s.filter(d => d.success).length,
        l = s.filter(d => d.error === "已取消").length;
    return lr({
        source: t,
        total: s.length,
        success: u,
        ...
        platforms: c,
        duration: Date.now() - n
    }).catch(() => {}), u > 0 && De("first_sync_success").catch(() => {}), dr(c).catch(() => {}), s
}
const Y = Te("WordPress");
// ============================================================================
// 【WordPress XML-RPC 实现】
// 通过 XML-RPC 协议与 WordPress 站点交互，支持自建博客发布：
//   ht(method, params): XML 请求构建器
//     - 将 JS 值序列化为 XML-RPC 参数格式（string/int/double/boolean/base64）
//   Kt(xml): XML 响应解析器
//     - 解析 XML-RPC 响应，提取返回值
//   ma(config): 认证测试
//     - 调用 wp.getUsersBlogs 验证用户名密码
//   ga(config, imageData, filename, mimeType): 图片上传
//     - 调用 wp.uploadFile，支持重试（最多 10 次）
//     - 处理 Base64 编码和图片 URL
//   Sa(config, images, article, uploadFn): 图片处理管线
//     - 遍历文章中所有图片，上传到 WordPress 媒体库
//     - 替换 HTML 和 Markdown 中的图片 URL
//   Ge(config, article, processedImages): 创建文章
//     - 调用 wp.newPost 或 metaWeblog.newPost
//     - 支持设置文章状态（draft/publish）和分类
// ============================================================================
function ht(c, o) {
...
function Kt(c) {
    if (c.includes("<fault>")) {
        ...
        return {
            success: !1,
            error: (e == null ? void 0 : e[1]) || "XML-RPC 错误"
        }
    }
...
async function ga(c, o, e, t) {
    ...
    try {
        const s = {
            ...
            n = ht("wp.uploadFile", [0, c.username, c.password, s]),
            i = await fetch(r, {
                method: "POST",
                headers: {
                    "Content-Type": "text/xml"
        ...
        if (a.includes("<fault>")) {
            ...
            return {
                success: !1,
                error: (l == null ? void 0 : l[1]) || "Upload failed"
            }
        }
        return {
            success: !1,
...
async function Ia(c, o, e) {
    ...
    try {
        ...
        return h.success && h.url ? {
            url: h.url
        } : (Y.error(` Failed to upload image: ${h.error}`), null)
    } catch (s) {
...
async function Sa(c, o, e, t) {
    ...
    let n;
    for (;
        (n = r.exec(o)) !== null;) s.push({
        full: n[0],
        src: n[1]
    });
    for (const h of xe(o)) s.push({
        full: h.full,
        src: h.src
    });
    if (s.length === 0) return {
    ...
    for (const {
        ...
        if (!f) {
            ...
            if (p != null && p.url) f = p.url, a.set(d, f);
            else {
...
async function Ge(c, o, e) {
    ...
    try {
        ...
        if ((s = e == null ? void 0 : e.signal) != null && s.aborted) return {
            success: !1,
            error: "操作已取消"
        };
        const a = {
...
// ============================================================================
// 【MetaWeblog XML-RPC 实现】
// 通过 XML-RPC 协议支持 Typecho、开源中国博客等 MetaWeblog 兼容平台：
//   Re(method, params): XML 请求构建器（与 WordPress 类似但参数格式略有不同）
//   Ea(config, imageData, filename, mimeType): 媒体上传
//     - 调用 metaWeblog.newMediaObject
//   Je(config, article, processedImages): 创建文章
//     - 调用 metaWeblog.newPost
//     - 支持标题、描述、分类、标签
//   Ke(config, article, processedImages): Typecho 专用
//     - 使用固定端点 /action/xmlrpc
//     - Typecho 特有的参数格式处理
// ============================================================================
function Re(c, o) {
...
function Ze(c) {
    if (c.includes("<fault>")) {
        ...
        return {
            success: !1,
            error: (r == null ? void 0 : r[1]) || "XML-RPC 错误"
        }
    }
    ...
    return t ? {
        success: !0,
        value: t[1]
    } : c.includes("<array>") || c.includes("<struct>") ? {
        success: !0,
        value: {}
    } : {
        success: !0
...
async function Ea(c, o, e, t) {
    ...
    try {
        ...
        if (u.includes("<fault>")) {
            ...
            return {
                success: !1,
                error: (h == null ? void 0 : h[1]) || "Upload failed"
            }
        }
        return {
            success: !1,
...
async function Oa(c, o, e) {
    ...
    for (let r = 1; r <= Oe; r++) {
        ...
        try {
            const s = await Pa(c, o, e);
            if (s) return r > 1 && W.debug(` Upload succeeded on attempt ${r}: ${o}`), s;
            W.warn(` Upload attempt ${r}/${Oe} failed for: ${o}`)
        } catch (s) {
            t = s, W.warn(` Upload attempt ${r}/${Oe} error:`, s)
        }
        if (r < Oe) {
            const s = Ua * r;
    ...
    return W.error(` All ${Oe} upload attempts failed for: ${o}`, t), null
}
async function Pa(c, o, e) {
    ...
    try {
        ...
        return h.success && h.url ? {
            url: h.url
        } : (W.error(` Failed to upload image: ${h.error}`), null)
    } catch (s) {
        throw clearTimeout(r), s
    }
}
...
async function Ke(c, o, e) {
    const t = c.url.replace(/\/$/, "") + "/action/xmlrpc",
        r = {
            ...c,
            endpoint: t
        };
...
async function Na(c) {
    ...
    if (c.status === "syncing") await chrome.action.setBadgeText({
        text: ""
    });
        ...
        c.selectedPlatforms.length - o === 0 ? (await chrome.action.setBadgeText({
            ...
            color: We.success
        })) : o === 0 ? (await chrome.action.setBadgeText({
            text: "!"
        }), await chrome.action.setBadgeBackgroundColor({
            color: We.error
        })) : (await chrome.action.setBadgeText({
            ...
            var s;
            ((s = (await chrome.storage.local.get(Ie))[Ie]) == null ? void 0 : s.status) === "completed" && await chrome.action.setBadgeText({
                text: ""
            })
        }, 5e3)
    } else(c.status === "failed" || c.status === "cancelled") && (await chrome.action.setBadgeText({
        text: "!"
    }), await chrome.action.setBadgeBackgroundColor({
        color: We.error
    }), setTimeout(async () => {
        ...
        (((e = o[Ie]) == null ? void 0 : e.status) === "failed" || ((t = o[Ie]) == null ? void 0 : t.status) === "cancelled") && await chrome.action.setBadgeText({
            text: ""
        })
    }, 5e3))
}
...
async function Fa(c, o, e) {
    try {
        const r = (await chrome.storage.local.get("syncHistory")).syncHistory || [],
            s = {
                id: c,
                status: "syncing",
                title: o.title || "未知文章",
        ...
        await chrome.storage.local.set({
            syncHistory: n
        }), ie.info("History created:", c, s.title)
    } catch (t) {
...
async function La(c, o, e, t) {
    try {
        const s = (await chrome.storage.local.get("syncHistory")).syncHistory || [],
            n = e.map(a => {
                var u;
                return {
                    ...a,
        ...
        await chrome.storage.local.set({
            syncHistory: i
        }), ie.info("History updated:", c, o)
    } catch (r) {
        ie.error("Failed to update history:", r)
    }
}
// ============================================================================
// 【同步服务 - Ha() (SyncService 入口)】
// 完整的同步流程入口，协调 DSL 平台和 CMS 账户的发布：
//   1. 分离目标: DSL 平台（内置适配器）vs CMS 账户（WordPress/Typecho 等）
//   2. 更新 Chrome 扩展图标 Badge（显示同步中平台数量）
//   3. 执行 DSL 平台同步（调用 ot() 编排器）
//   4. 执行 CMS 同步（调用 WordPress/MetaWeblog XML-RPC）
//   5. 记录同步历史（最多保留 25 条记录）
// qa(content, tabId, source): 内容预处理
//   - 方式一: 通过已有的 tab 通信（content-script 消息）
//   - 方式二: 创建临时 popup 窗口执行预处理
//   - 输出: 预处理后的 HTML + Markdown + 元数据
// ============================================================================
async function Ha(c, o, e = {}, t = {}) {
    ...
    const {
        ...
        onImageProgress: i,
        onDetailProgress: a
    } = t, u = Pe(), l = new Map(u.map(b => [b.id, b.name])), h = ja(), d = {
        title: c.title,
        content: c.content || c.html || "",
        html: c.html || c.content || "",
        markdown: c.markdown || "",
        cover: c.cover
    }, f = (await chrome.storage.local.get("cmsAccounts")).cmsAccounts || [], p = new Set(f.map(b => b.id)), w = o.filter(b => !p.has(b)), _ = o.filter(b => p.has(b)), y = {
        syncId: h,
        status: "syncing",
        article: {
            title: d.title,
    ...
    const T = (O = c.source) == null ? void 0 : O.platform,
        P = w.filter(b => b !== T);
    let A = d;
    if (P.length > 0) {
        const b = at(P);
        if (s === "mcp")
            for (const C of Object.keys(b)) b[C] = {
                ...b[C],
                keepStyles: !0
            };
        const I = d.html || d.content || "";
        if (I) {
            const C = await qa(I, P, b);
            C ? (A = {
                ...d,
                platformContents: C
            }, ie.debug("Preprocessed for platforms:", Object.keys(C))) : ie.warn("DOM preprocessing unavailable — please ensure at least one web page is open in Chrome")
        }
    }
    const S = [];
    w.length > 0 && await ot(w, A, {
        onResult: b => {
            const I = {
                ...b,
                platformName: l.get(b.platform) || b.platform
            };
            y.results.push(I), S.push(I), me(y).catch(() => {}), n == null || n(I)
        },
        onImageProgress: (b, I, C) => {
            i == null || i(b, I, C)
        },
        onDetailProgress: b => {
            a == null || a(b)
    ...
    for (const b of _) {
        const I = f.find(C => C.id === b);
        if (!I) {
            const C = {
                platform: b,
                platformName: l.get(b) || b,
                success: !1,
                error: "CMS 账户不存在"
            };
            S.push(C), y.results.push(C), me(y).catch(() => {}), n == null || n(C), a == null || a({
                platform: b,
                platformName: C.platformName || b,
                stage: "failed",
                error: C.error
            });
            continue
        ...
        try {
            ...
            if (!F) {
                ...
                S.push(E), y.results.push(E), me(y).catch(() => {}), n == null || n(E), a == null || a({
                    platform: b,
                    platformName: I.name,
                    stage: "failed",
                    error: "密码未找到"
            ...
            S.push(x), y.results.push(x), me(y).catch(() => {}), n == null || n(x), a == null || a({
                platform: b,
                platformName: I.name,
            ...
            S.push(F), y.results.push(F), me(y).catch(() => {}), n == null || n(F), a == null || a({
                platform: b,
                platformName: I.name,
                stage: "failed",
                error: C.message
    ...
    const v = S.filter(b => b.success).length,
        U = S.length - v,
        k = S.length === 0 && o.length > 0 || U === S.length && S.length > 0 ? "failed" : "completed";
    return y.status = k, await me(y), r || await La(h, k, S, u), {
        results: S,
        syncId: h
    }
}
// ============================================================================
// 【内容预处理 - qa()】
// 将原始 HTML 内容转换为各平台所需格式：
//   输入: 原始 HTML + tabId + source（来源标识）
//   流程:
//     1. 如果有 tabId，向 content-script 发送 PREPROCESS_CONTENT 消息
//     2. 否则创建临时 popup 窗口加载页面并执行预处理
//     3. 应用预处理器配置（Ir 默认配置 + 平台特定覆盖）
//   输出: { html, markdown, title, meta }
// ============================================================================
...
const H = Te("MCPClient"),
    $t = "ws://localhost:9527";
// ============================================================================
// 【MCP WebSocket 客户端 - Wa (MCPClient)】
// Model Context Protocol 客户端，允许外部 AI 工具通过 WebSocket 调用扩展功能。
// 连接地址: ws://localhost:9527（本地 MCP 服务器）
// 认证: 使用 Token 验证客户端身份
// 重连策略:
//   - 暖重连: 短暂断线（< 30s），指数退避重试
//   - 冷重连: 长时间断线，重置重连计数
// 支持的 MCP 方法:
//   - listPlatforms: 返回所有支持的平台列表
//   - checkAuth: 检查指定平台的登录状态
//   - syncArticle: 同步文章到指定平台（调用 Ha()）
//   - extractArticle: 从 URL 提取文章内容
//   - uploadImage: 上传图片到指定平台（支持分块上传）
//     * start: 初始化上传
//     * chunk: 上传数据块
//     * complete: 完成上传
...
class Wa {
    ...
    async handleMethod(o, e) {
        ...
        switch (o) {
            ...
            case "syncArticle": {
                const s = e == null ? void 0 : e.platforms,
                    n = e == null ? void 0 : e.article;
            ...
            case "extractArticle": {
                ...
                const n = s[0].id;
                try {
                    const i = await chrome.tabs.sendMessage(n, {
                        type: "EXTRACT_ARTICLE"
                    });
                    return (i == null ? void 0 : i.article) || null
                } catch {
                    ...
                    const a = (r = chrome.runtime.getManifest().content_scripts) == null ? void 0 : r.flatMap(l => l.js || []).find(l => l.includes("extractor"));
                    ...
                    const u = await chrome.tabs.sendMessage(n, {
                        type: "EXTRACT_ARTICLE"
                    });
                    return (u == null ? void 0 : u.article) || null
                }
            }
            case "uploadImage": {
                const s = e == null ? void 0 : e.imageData,
                    n = e == null ? void 0 : e.mimeType,
            ...
            case "uploadImage:start": {
                const s = e == null ? void 0 : e.uploadId,
                    n = e == null ? void 0 : e.totalChunks,
                    i = e == null ? void 0 : e.mimeType,
                    a = (e == null ? void 0 : e.platform) || "weibo";
                ...
                return this.pendingUploads.set(s, {
                    ...
                    createdAt: Date.now(),
                    timeoutId: u
                }), H.debug(`Chunked upload started: ${s}, ${n} chunks`), {
                    success: !0
                }
            }
            case "uploadImage:chunk": {
                const s = e == null ? void 0 : e.uploadId,
                    n = e == null ? void 0 : e.chunkIndex,
...
async function Ja(c) {
    ...
    if (c.status === "syncing") await chrome.action.setBadgeText({
        ...
        c.selectedPlatforms.length - o === 0 ? (await chrome.action.setBadgeText({
            ...
            var s;
            ((s = (await chrome.storage.local.get(Z))[Z]) == null ? void 0 : s.status) === "completed" && await chrome.action.setBadgeText({
                text: ""
            })
        }, 5e3)
    }
...
async function Ka() {
    await chrome.storage.local.remove(Z), await chrome.action.setBadgeText({
        text: ""
    })
}
chrome.runtime.onMessage.addListener((c, o, e) => (Ya(c, o).then(e).catch(t => e({
    error: t.message
})), !0));
// ============================================================================
// 【Chrome 消息处理器 - Ya() (MessageHandler)】
// 统一处理来自 popup、content-script、MCP 客户端的所有消息：
// 消息类型及处理逻辑:
//   平台管理:
//     - GET_PLATFORMS: 返回所有已注册平台及其认证状态
//     - CHECK_ALL_AUTH: 批量检查所有平台登录态（调用 Ne()）
//     - CHECK_AUTH: 检查单个平台登录态
//   同步发布:
//     - SYNC_ARTICLE: 完整同步流程（预处理 + 并行发布 + CMS 同步）
//     - START_SYNC_FROM_EDITOR: 从编辑器页面发起同步
//     - OPEN_SYNC_PAGE: 打开同步选择页面
//     - CANCEL_SYNC: 取消正在进行的同步（调用 da()）
//     - GET_SYNC_STATE: 获取当前同步状态
//     - CLEAR_SYNC_STATE: 清除同步状态
//     - UPDATE_SYNC_STATUS: 更新同步进度
//   CMS 系统:
//     - TEST_CMS_CONNECTION: 测试 WordPress/Typecho 连接
//     - SYNC_TO_CMS: 发布到 CMS 系统
//   MCP 管理:
//     - MCP_ENABLE/DISABLE: 启用/禁用 MCP 服务
//     - MCP_SET_SERVER_URL: 设置 MCP 服务器地址
//     - MCP_WATCH_START/STOP: 开始/停止监听 MCP 事件
//     - MCP_STATUS: 获取 MCP 连接状态
//   其他:
//     - UPLOAD_IMAGE: 上传图片到指定平台
//     - MAGIC_CALL: 执行特殊调用（平台特定的自定义操作）
//     - GET_PREPROCESS_CONFIGS: 获取所有平台的预处理器配置
//     - CLEAR_UPDATE_BADGE: 清除更新提示标记
//     - TRIGGER_OPEN_EDITOR: 触发打开平台编辑器
//     - TRACK_ARTICLE_EXTRACT: 追踪文章提取事件
// ============================================================================
    ...
    switch (c.type) {
        ...
        case "SYNC_ARTICLE": {
            const {
                ...
                source: d = "popup",
                syncId: m
            } = c.payload, f = Pe(), p = m || Ot(), w = (t = o == null ? void 0 : o.tab) == null ? void 0 : t.id, _ = x => {
                const E = {
                    ...x,
                    syncId: p
                };
                chrome.runtime.sendMessage(E).catch(() => {}), w && chrome.tabs.sendMessage(w, E).catch(() => {})
            }, y = await mt(u), P = (await chrome.storage.local.get("cmsAccounts")).cmsAccounts || [], A = new Set(P.map(x => x.id)), S = u.filter(x => !A.has(x)), v = u.filter(x => A.has(x)), U = (r = a.source) == null ? void 0 : r.platform, k = S.filter(x => x !== U);
            let O = a;
            ...
            await K(b), h || await Pt(p, a, u);
            const I = [];
            S.length > 0 && await ot(S, O, {
                onResult: x => {
                    var q;
                    const E = {
                        ...x,
                    ...
                    b.results.push(E), I.push(E), K(b).catch(() => {}), _({
                        type: "SYNC_PROGRESS",
                        payload: {
                            result: E
                        }
                ...
                if (E) {
                    ...
                    try {
                        ...
                        if (!M) {
                            ...
                            I.push(Le), b.results.push(Le), K(b).catch(() => {}), _({
                                type: "SYNC_PROGRESS",
                                payload: {
                                    result: Le
                                }
                        ...
                        I.push(Fe), b.results.push(Fe), K(b).catch(() => {}), _({
                            type: "SYNC_PROGRESS",
                            payload: {
                                result: Fe
                            }
                        ...
                        I.push(M), b.results.push(M), K(b).catch(() => {}), _({
                            type: "SYNC_PROGRESS",
                            payload: {
                                result: M
                            }
            ...
            const C = I.filter(x => x.success).length,
                J = I.length - C === I.length ? "failed" : "completed";
        ...
        case "OPEN_SYNC_PAGE": {
            const a = c.path || "",
                u = chrome.runtime.getURL("src/popup/index.html") + (a ? `#${a}` : ""),
                l = 396,
                h = 560,
                d = await chrome.windows.getCurrent(),
        ...
        case "TEST_CMS_CONNECTION": {
            ...
            try {
                ...
                return ft("test", a, m.success).catch(() => {}), m
            } catch (m) {
                return ft().catch(() => {}), {
                    success: !1,
                    error: m.message
        ...
        case "SYNC_TO_CMS": {
            ...
            try {
                ...
                mr("popup", d.type, p.success).catch(() => {}), p.success ? De("cms_user").catch(() => {}) : p.error && gr("cms_sync_error", {
                    cms_type: d.type,
                    error_type: jt(p.error)
                }).catch(() => {});
        ...
        case "MCP_ENABLE": {
            ...
            return await chrome.storage.local.set({
                mcpEnabled: !0,
                mcpToken: u
            }), G.setToken(u), a.mcpServerUrl && G.setServerUrl(a.mcpServerUrl), Vt(), j.info(" MCP enabled"), gt().catch(() => {}), De("mcp_user").catch(() => {}), {
                success: !0,
                token: u
            }
        }
        case "MCP_DISABLE":
            return await chrome.storage.local.set({
                mcpEnabled: !1
            }), G.clearToken(), Xa(), j.info(" MCP disabled"), gt().catch(() => {}), {
                success: !0
            };
        case "MCP_SET_SERVER_URL": {
            ...
            return await chrome.storage.local.set({
                mcpServerUrl: a || ""
            }), G.setServerUrl(a), G.isConnected() && G.disconnect(), G.resetReconnect(), {
                success: !0
            }
        }
        case "MCP_WATCH_START":
            return G.setActivelyWatched(!0), {
                success: !0
            };
        case "MCP_WATCH_STOP":
            return G.setActivelyWatched(!1), {
                success: !0
            };
        case "MCP_STATUS": {
            const a = await chrome.storage.local.get(["mcpEnabled", "mcpToken", "mcpServerUrl"]),
                u = Ga();
            return {
        ...
        case "GET_SYNC_STATE":
            return {
                syncState: (await chrome.storage.local.get(Z))[Z] || null
            };
        ...
        case "UPDATE_SYNC_STATUS": {
            const {
                status: a
            } = c.payload, l = (await chrome.storage.local.get(Z))[Z];
            return l && (l.status = a, await K(l)), {
                success: !0
            }
        }
        case "CANCEL_SYNC": {
            const a = da();
            return j.info("Sync cancelled:", a), {
                success: a
            }
        }
        case "START_SYNC_FROM_EDITOR": {
            const {
                ...
                platforms: u,
                syncId: l
            } = c, h = (s = o == null ? void 0 : o.tab) == null ? void 0 : s.id, d = Pe();
            if (!h) return {
                error: "No tab ID found"
            };
            const m = l || Ot(),
                f = b => {
                    chrome.tabs.sendMessage(h, {
                        ...b,
                        syncId: m
                    }).catch(() => {})
                },
                p = await mt(u),
                _ = (await chrome.storage.local.get("cmsAccounts")).cmsAccounts || [],
                y = new Set(_.map(b => b.id)),
                T = u.filter(b => !y.has(b)),
                P = u.filter(b => y.has(b)),
                A = {
                    syncId: m,
                    status: "syncing",
                    article: {
            ...
            T.length > 0 && await ot(T, a, {
                onResult: b => {
                    ...
                    const I = {
                        ...b,
                        platformName: ((C = d.find(F => F.id === b.platform)) == null ? void 0 : C.name) || b.platform
                    };
                    A.results.push(I), S.push(I), K(A).catch(() => {}), f({
                        type: "SYNC_PROGRESS",
                        result: I
                    })
                },
                ...
                if (I) {
                    ...
                    try {
                        ...
                        if (!F) {
                            ...
                            S.push(E), A.results.push(E), K(A).catch(() => {}), f({
                                type: "SYNC_PROGRESS",
                                result: E
                            }), f({
                                type: "SYNC_DETAIL_PROGRESS",
                        ...
                        S.push(x), A.results.push(x), K(A).catch(() => {}), f({
                            type: "SYNC_PROGRESS",
                            result: x
                        }), f({
                            type: "SYNC_DETAIL_PROGRESS",
                        ...
                        S.push(F), A.results.push(F), K(A).catch(() => {}), f({
                            type: "SYNC_PROGRESS",
                            result: F
                        }), f({
                            type: "SYNC_DETAIL_PROGRESS",
            ...
            const v = S.filter(b => b.success).length,
                k = S.length - v === S.length ? "failed" : "completed";
            A.status = k, await K(A), f({
                type: "SYNC_COMPLETE",
                rateLimitWarning: p
            }), await Dt(m, k, S, d);
            const O = S.filter(b => b.success).map(b => b.platform);
            return O.length > 0 && et(O).catch(() => {}), {
                results: S,
                rateLimitWarning: p,
                syncId: m
            }
        ...
        case "UPLOAD_IMAGE": {
            ...
            try {
                const l = await je(u);
                if (!l) return {
                    error: `Platform not found: ${u}`
                };
                if (typeof l.uploadImage != "function") return {
                    error: `Platform ${u} does not support image upload`
                };
                let h;
                ...
                return {
                    result: {
                        url: await l.uploadImage(h),
                        platform: u
                    }
                }
            } catch (l) {
                return j.error("Upload image failed:", l), {
                    error: l.message
                }
            }
        }
        case "MAGIC_CALL": {
            ...
            try {
                const l = ((n = u.account) == null ? void 0 : n.type) || u.platform || "weibo",
                    h = await je(l);
                return h ? typeof h[a] != "function" ? {
                    error: `Method ${a} not found on platform ${l}`
                } : {
                    result: await h[a](u)
                } : {
                    error: `Platform not found: ${l}`
                }
            } catch (l) {
                return j.error(`Magic call ${a} failed:`, l), {
                    error: l.message
                }
            }
        }
        case "CLEAR_UPDATE_BADGE":
            return await chrome.action.setBadgeText({
                text: ""
            }), j.info("Update badge cleared"), {
                success: !0
            };
        case "GET_PREPROCESS_CONFIGS":
            return await Me(), {
                configs: at(c.platforms)
            };
...
// 【生命周期管理】
// 上下文菜单 (contextMenus):
//   - 右键菜单"同步到..."，在网页上选择文本后出现
//   - 点击后提取当前页面内容并打开同步页面
//
// onInstalled 事件:
//   - 更新时: 打开更新日志页面
//   - 首次安装时: 打开欢迎页面
//   - 初始化: MCP 自动连接（如果之前启用过）、认证预检查
//
// onStartup 事件:
//   - 清理孤立的 declarativeNetRequest 规则
//   - 重新初始化认证缓存
//
// 定时任务 (alarms):
//   - daily_metrics: 每日上报使用统计
//   - remote_config: 每 6 小时拉取远程配置（版本更新、功能开关）
//
// 更新标记:
//   - 检测新版本后在扩展图标上显示 "NEW" 提示
//   - 用户点击后清除标记（CLEAR_UPDATE_BADGE 消息）
// ============================================================================
chrome.contextMenus.onClicked.addListener(async (c, o) => {
    if (c.menuItemId === "wechatsync-open-editor" && (o != null && o.id)) try {
        const t = (await Ne(!1)).map(a => ({
                ...a,
                sourceType: "dsl"
            })),
...
chrome.runtime.onInstalled.addListener(async c => {
    if (j.info(" Installed:", c.reason, c.previousVersion), Va(), await Me(), fr(c.reason, c.previousVersion).catch(() => {}), Ft().catch(() => {}), c.reason === "install" && wr().catch(() => {}), c.reason === "update") {
        const o = c.previousVersion || "0.0.0",
            e = chrome.runtime.getManifest().version;
        (["2.0.8", "2.0.9"].includes(e) || o.startsWith("1.") && e.startsWith("2.")) && chrome.tabs.create({
            url: "https://www.wechatsync.com/changelog?from=" + o + "&to=" + e,
            active: !0
        })
    }
    c.reason === "install" && chrome.tabs.create({
...
async function Qa() {
    ...
    if (c.mcpEnabled) {
        if (c.mcpToken) G.setToken(c.mcpToken), j.info(" Starting MCP client with existing token...");
        else {
            const o = crypto.randomUUID();
            await chrome.storage.local.set({
                mcpToken: o
            }), G.setToken(o), j.info(" Starting MCP client with new token...")
        }
...
Qt();
chrome.alarms.create("daily_growth_metrics", {
    periodInMinutes: 24 * 60
});
chrome.alarms.create("remote_config_fetch", {
    periodInMinutes: 6 * 60
});
chrome.alarms.onAlarm.addListener(c => {
    c.name === "daily_growth_metrics" && Nt().catch(() => {}), c.name === "remote_config_fetch" && Ft().catch(() => {})
});
Nt().catch(() => {});
_r().catch(() => {});
yr().then(async c => {
    c.hasUpdate && c.info && (await br(c.info.version) || (await chrome.action.setBadgeText({
        text: "NEW"
    }), await chrome.action.setBadgeBackgroundColor({
        color: Ye.update
    }), j.info("Update badge shown for version:", c.info.version)))
}).catch(() => {});
async function Za() {
    try {
        const c = await chrome.declarativeNetRequest.getDynamicRules();
        c.length > 0 && (j.info(`Clearing ${c.length} orphaned dynamic rules...`), await chrome.declarativeNetRequest.updateDynamicRules({
            removeRuleIds: c.map(o => o.id)
        }), j.info("Orphaned rules cleared"))
    } catch (c) {
...
async function Pt(c, o, e) {
    try {
        ...
        await chrome.storage.local.set({
            syncHistory: n
        }), j.info("History created:", c, s.title)
    } catch (t) {
