...
function Z() {
    const [a, N] = r.useState(null), [j, F] = r.useState([]), [o, c] = r.useState([]), [d, m] = r.useState("idle"), [u, x] = r.useState([]), [E, h] = r.useState(null), [L, v] = r.useState(null), [O, p] = r.useState(new Map), [T, y] = r.useState(null), b = r.useRef(null), [M, R] = r.useState(!1);
    r.useEffect(() => {
        b.current = T
    }, [T]);
        ...
        const n = l => {
            try {
                ...
                if (t.syncId) {
                    ...
                    else if (t.syncId !== b.current) {
                        g.debug("Ignoring message with different syncId:", t.syncId, "current:", b.current);
                        return
                    }
                }
                if (g.debug("Received message:", t), t.type === "ARTICLE_DATA") N(t.article), w.current && t.article.content && (w.current.innerHTML = t.article.content);
                else if (t.type === "PLATFORMS_DATA") F(t.platforms), t.selectedPlatformIds && t.selectedPlatformIds.length > 0 ? (c(t.selectedPlatformIds), S(t.selectedPlatformIds)) : chrome.storage.local.get(P).then(s => {
                    const i = s[P],
                        $ = t.platforms.filter(f => f.isAuthenticated).map(f => f.id),
                        G = new Set($),
                        q = i ? i.filter(f => G.has(f)) : [];
                    c(q)
                }).catch(s => {
                    g.error("Failed to load selected platforms:", s), c([])
                });
                else if (t.type === "SYNC_PROGRESS") t.result && x(s => [...s, t.result]);
                else if (t.type === "SYNC_DETAIL_PROGRESS") {
                    const s = t.progress;
                    s != null && s.platform && p(i => {
                        const C = new Map(i);
                        return C.set(s.platform, s), C
                    })
                } else t.type === "SYNC_COMPLETE" ? (m("completed"), t.rateLimitWarning && (v(t.rateLimitWarning), setTimeout(() => v(null), 8e3))) : t.type === "SYNC_ERROR" && (h(t.error), m("idle"))
            } catch (t) {
                g.error("Failed to parse message:", t)
            }
        };
        return window.addEventListener("message", n), window.parent.postMessage(JSON.stringify({
            type: "EDITOR_READY"
        }), "*"), () => window.removeEventListener("message", n)
    }, []), r.useEffect(() => {
        d === "syncing" && u.length > 0 && u.length >= o.length && m("completed")
    }, [u.length, o.length, d]);
    const z = r.useCallback(() => {
        ...
        D = r.useCallback(() => {
            ...
            return a ? {
                ...a,
                title: ((n = _.current) == null ? void 0 : n.innerText) || a.title,
                content: ((l = w.current) == null ? void 0 : l.innerHTML) || a.content
            } : null
        }, [a]),
        B = n => {
            c(l => {
        ...
        H = () => {
            ...
            y(l), m("syncing"), x([]), h(null), p(new Map), window.parent.postMessage(JSON.stringify({
                type: "START_SYNC",
                article: n,
                platforms: o,
                syncId: l
        ...
        J = () => {
            ...
            y(t), m("syncing"), x(s => s.filter(i => i.success)), p(new Map), window.parent.postMessage(JSON.stringify({
                type: "START_SYNC",
                article: l,
                platforms: n,
                syncId: t
    ...
    if (!a) return e.jsx("div", {
        ...
        children: e.jsxs("div", {
            ...
            children: [e.jsx(K, {
                className: "w-8 h-8 animate-spin text-gray-400 mx-auto"
            }), e.jsx("p", {
        ...
        children: [e.jsx("header", {
            ...
            children: e.jsxs("div", {
                ...
                children: [e.jsxs("div", {
                    ...
                    children: [e.jsx("img", {
                        ...
                        children: "同步助手 - 点击内容可直接修改"
                    }), (a == null ? void 0 : a.extractor) && e.jsx("span", {
                    ...
                    children: [e.jsxs("button", {
                        onClick: () => R(!0),
                        className: U("px-4 py-2 rounded-lg font-medium transition-colors", k > 0 ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-gray-200 text-gray-400 cursor-not-allowed"),
                        disabled: k === 0,
            ...
            children: e.jsxs("div", {
                ...
                children: [e.jsx("span", {
                    ...
                    className: "text-sm text-yellow-800 flex-1",
                    children: L
                }), e.jsx("button", {
                    onClick: () => v(null),
