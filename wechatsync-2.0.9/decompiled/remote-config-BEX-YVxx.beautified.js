...
const c = u("RemoteConfig"),
    h = "https://wpics.oss-cn-shanghai.aliyuncs.com/wechatsync-config.json",
    i = "remoteBanners",
    f = "remoteBanners_lastFetch",
    r = "dismissedBanners",
    m = 6;
...
function d(t, e) {
    ...
    for (let a = 0; a < 3; a++) {
        if ((n[a] || 0) > (o[a] || 0)) return !0;
        if ((n[a] || 0) < (o[a] || 0)) return !1
    }
    return !0
}
...
function g(t) {
    const e = Date.now();
    return !(t.startDate && new Date(t.startDate).getTime() > e || t.endDate && new Date(t.endDate).getTime() < e)
}
async function l() {
    try {
        ...
        const e = await t.json(),
            ...
            o = (e.banners || []).filter(n => g(n)).filter(n => !n.targetVersion || d(n.targetVersion, s)).sort((n, a) => (a.priority || 0) - (n.priority || 0));
...
async function D(t) {
    try {
        ...
        s.includes(t) || await chrome.storage.local.set({
            [r]: [...s, t]
        })
    } catch {}
}
export {
    p as a, D as d, l as f, y as g
};